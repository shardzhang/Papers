...
2:     对于层 l 中的所有胶囊 i 和层 (l+1) 中的所有胶囊 j：b_{ij} ← 0
3:     重复 r 次迭代 do
4:         对于层 l 中的所有胶囊 i：c_i ← softmax(b_i)         ▷ softmax 计算式 (3)
5:         对于层 (l+1) 中的所有胶囊 j：s_j ← ∑_i c_{ij} û_{j|i}
6:         对于层 (l+1) 中的所有胶囊 j：v_j ← squash(s_j)      ▷ squash 计算式 (1)
7:         对于层 l 中的所有胶囊 i 和层 (l+1) 中的所有胶囊 j：b_{ij} ← b_{ij} + û_{j|i} · v_j
8:     return v_j
```
