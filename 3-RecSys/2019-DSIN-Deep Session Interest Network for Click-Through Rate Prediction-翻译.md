...
**嵌入**
嵌入是一种将大规模稀疏特征转换为低维稠密向量的常用技术。数学上，稀疏特征可以分别表示为E \in R^(M×d_model)，其中M是稀疏特征的大小，d_model是嵌入大小。通过嵌入，用户画像可以表示为X_U \in R^(N_u×d_model)，其中N_u是用户画像的稀疏特征数量。item画像可以表示为X_I \in R^(N_i×d_model)，其中N_i是item画像的稀疏特征数量。用户行为可以表示为S = [b_1; ...; b_i; ...; b_N] \in R^(N×d_model)，其中N是用户历史行为的数量，b_i是第i个行为的嵌入。
...
**会话划分层**
为了提取更精确的用户会话兴趣，我们将用户的行为序列S划分为会话Q，其中第k个会话Q_k = [b_1; ...; b_i; ...; b_T] \in R^(T×d_model)，T是我们在会话中保留的行为数量，b_i是用户在该会话中的第i个行为。用户会话的分割发生在时间间隔超过30分钟的相邻行为之间，遵循[Grbovic and Cheng, 2018]。
...
**偏置编码。** 为了利用序列的顺序关系，自注意力机制对输入嵌入应用位置编码。此外，会话的顺序关系以及不同表示子空间中存在的偏置也需要被捕捉。因此，我们在位置编码的基础上提出了偏置编码BE \in R^(K×T×d_model)，其中BE中的每个元素定义如下：
...
**多头自注意力。** 在推荐系统中，用户的点击行为受到多种因素（例如颜色、风格和价格）的影响[Zhou et al., 2018a]。多头自注意力可以捕捉不同表示子空间中的关系。数学上，设Q_k = [Q_k1; ...; Q_kh; ...; Q_kH]，其中Q_kh \in R^(T×d_h)是Q_k的第h个头，H是头的数量，d_h = 1/h * d_model。第h个头的输出计算如下：
