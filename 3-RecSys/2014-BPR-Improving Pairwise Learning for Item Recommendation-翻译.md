...
> Steffen Rendle, Christoph Freudenthaler | University of Konstanz, Germany

本文提出了面向隐式反馈item推荐的自适应成对采样算法，通过非均匀采样信息性pair来加速BPR的收敛。核心内容：

- **问题识别**：item流行度的长尾分布导致均匀采样BPR收敛缓慢
...
$$
p(j|c) \propto |\{(c', j^{\prime}) \in S : j = j'\}|. \qquad (10)
$$
...
$$
p(j|c) \propto |\{(c', j^{\prime}) \in S : c = c', j = j'\}| = \delta((c, j) \in S). \qquad (13)
$$
...
其中 $p(f|c)$ 是概率函数：
$$
p(f|c) \propto |v_{c,f}| \sigma_f \qquad (17)
$$
