...
其中 xembed,i 是嵌入向量，xi 是第 i 个类别中的二值输入，Wembed,i \in R^n_e × n_v 是相应的嵌入矩阵，该矩阵将与网络中的其他参数一起优化，n_e 和 n_v 分别是嵌入大小和词表大小。
...
其中 h_l \in R^n_l, $h_{l+1}$ \in R^$n_{l+1}$ 分别是第 l 层和第 (l+1) 层隐藏层；W_l \in R^$n_{l+1}$×n_l, b_l \in R^$n_{l+1}$ 是第 l 个深度层的参数；f(·) 是 ReLU 函数。
...
**数据处理和嵌入。** 实值特征通过对数变换进行归一化。对于类别特征，我们将其嵌入到维度为 6 × (类别基数)^{1/4} 的稠密向量中。拼接所有嵌入向量后得到一个维度为 1026 的向量。
...
对于每个模型的最优超参数设置，我们还报告了 10 次独立运行中测试 logloss 的均值和标准差：DCN: 0.4422 ± 9×10⁻⁵, DNN: 0.4430 ± 3.7×10⁻⁴, DC: 0.4430 ± 4.3×10⁻⁴。可以看出，DCN 始终大幅优于其他模型。
...
| Logloss | DNN | DCN |
|---------|-----|-----|
| 0.4430  | 3.2×10⁶ | 7.9×10⁵ |
| 0.4460  | 1.5×10⁵ | 7.3×10⁴ |
| 0.4470  | 1.5×10⁵ | 3.7×10⁴ |
| 0.4480  | 7.8×10⁴ | 3.7×10⁴ |
...
**表 4：** DCN 与 DNN 之间的验证 logloss 差异（×10⁻²）。DNN 模型是将交叉层数设为 0 的 DCN 模型。负值表示 DCN 优于 DNN。
...
g_l(x_0) = x^T_l w_l = \Sigma^{l+1}_{p=1} \Sigma_{|i|=p} Π^$l_{j=0}$ (x^T_0 w_j)^{i_j},                    (9)
...
g_k(x_0) = x^T_k w_k = \Sigma^{k+1}_{p=1} \Sigma_{|i|=p} Π^$k_{j=0}$ (x^T_0 w_j)^{i_j}。
...
x^$T_{k+1}$ $w_{k+1}$ = (\Sigma^{k+1}_{p=1} \Sigma_{|i|=p} Π^$k_{j=0}$ (x^T_0 w_j)^{i_j}) (x^T_0 $w_{k+1}$) + \Sigma^{k+1}_{p=1} \Sigma_{|i|=p, i_k=0} Π^{k-1}_{j=0} (x^T_0 w_j)^{i_j} + (x^T_0 $w_{k+1}$) + Π^$k_{j=0}$ (x^T_0 w_j)

= \Sigma^{k+2}_{p=2} \Sigma_{|i|=p, i_k=1} Π^{k+1}_{j=0} (x^T_0 w_j)^{i_j} + \Sigma^{k+1}_{p=1} \Sigma_{|i|=p, i_k=0} Π^{k+1}_{j=0} (x^T_0 w_j)^{i_j}

= \Sigma^{k+2}_{p=1} \Sigma_{|i|=p} Π^{k+1}_{j=0} (x^T_0 w_j)^{i_j}.                                     (11)
...
c_\alpha = \Sigma_{j_1,...,j_p\inP_p} \Sigma_{i_1,...,i_p\inB_p} Π^$p_{k=1}$ $w_{j_k}$^{(i_k)}.                          (12)
