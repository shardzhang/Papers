...
其中 xembed,i 是嵌入向量，xi 是第 i 个类别中的二值输入，Wembed,i ∈ R^n_e × n_v 是相应的嵌入矩阵，该矩阵将与网络中的其他参数一起优化，n_e 和 n_v 分别是嵌入大小和词表大小。
...
其中 x_l, x_l+1 ∈ R^d 是列向量，分别表示第 l 层和第 (l+1) 层交叉层的输出；w_l, b_l ∈ R^d 是第 l 层的权重和偏置参数。每个交叉层在特征交叉 f 之后加回其输入，映射函数 f : R^d → R^d 拟合 x_l+1 − x_l 的残差。一个交叉层的可视化如图 2 所示。

**特征间的高阶交互。** 交叉网络的特殊结构导致交叉特征的程度随层深度增长。对于一个 l 层交叉网络，最高多项式度数（就输入 x0 而言）为 l+1。实际上，交叉网络包含了所有度数为 1 到 l+1 的交叉项 x1^α1 x2^α2 ... xd^αd。详细分析见第 3 节。
...
其中 h_l ∈ R^n_l, h_{l+1} ∈ R^n_{l+1} 分别是第 l 层和第 (l+1) 层隐藏层；W_l ∈ R^n_{l+1}×n_l, b_l ∈ R^n_{l+1} 是第 l 个深度层的参数；f(·) 是 ReLU 函数。
...
p = σ([x^T_{L1}, h^T_{L2}] w_logits),                           (5)

其中 x_{L1} ∈ R^d, h_{L2} ∈ R^m 分别是交叉网络和深度网络的输出，w_logits ∈ R^{d+m} 是组合层的权重向量，σ(x) = 1/(1 + exp(−x))。

损失函数是 log loss 加上一个正则化项：

loss = − 1/N Σ^N_{i=1} [y_i log(p_i) + (1 − y_i) log(1 − p_i)] + λ Σ_l ||w_l||^2, (6)

其中 p_i 是由公式 5 计算出的概率，y_i 是真实标签，N 是输入总数，λ 是 L2 正则化参数。
...
**符号。** 令 w_j 中的第 i 个元素为 w_j^(i)。对于多重索引 α = [α_1, ..., α_d] ∈ N^d 和 x = [x_1, ..., x_d] ∈ R^d，我们定义 |α| = Σ^d_{i=1} α_i。

**术语。** 交叉项（单项式）x_1^α1 x_2^α2 ... x_d^αd 的度数定义为 |α|。多项式的度数由其项的最高度数定义。
...
P_n(x) = { Σ_α w_α x_1^α1 x_2^α2 ... x_d^αd | 0 ≤ |α| ≤ n, α ∈ N^d }.     (7)

该类中的每个多项式有 O(d^n) 个系数。我们证明，仅用 O(d) 个参数，交叉网络就包含了相同度数多项式中出现的所有交叉项，且每个项的系数各不相同。

**定理 3.1.** 考虑一个 l 层交叉网络，其第 i+1 层定义为 x_{i+1} = x_0 x^T_i w_i + x_i。令网络输入为 x_0 = [x_1, x_2, ..., x_d]^T，输出为 g_l(x_0) = x^T_l w_l，参数为 w_i, b_i ∈ R^d。那么，多元多项式 g_l(x_0) 再现了以下类中的多项式：

{ Σ_α c_α(w_0, ..., w_l) x_1^α1 x_2^α2 ... x_d^αd | 0 ≤ |α| ≤ l+1, α ∈ N^d },

其中 c_α = M_α Σ_{i∈Bα} Σ_{j∈Pα} Π^{|α|}_{k=1} w_{j_k}^{(i_k)}，M_α 是一个与 w_i 无关的常数，i = [i_1, ..., i_{|α|}] 和 j = [j_1, ..., j_{|α|}] 是多索引，Bα = { y ∈ {0, 1, ..., l}^{|α|} | y_i < y_j ∧ y_{|α|} = l }，Pα 是索引 (1, ..., 1, ..., d, ..., d) 的所有排列的集合。
   |_α1_|      |_αd_|

定理 3.1 的证明在附录中。让我们给出一个例子。考虑 x_1 x_2 x_3 的系数 c_α，其中 α = (1, 1, 1, 0, ..., 0)。当 l=2 时，c_α = Σ_{i,j,k∈Pα} w_0^(i) w_1^(j) w_2^(k) + w_0^(i) w_2^(j) w_1^(k) + w_1^(i) w_0^(j) w_2^(k) + w_1^(i) w_2^(j) w_0^(k) + w_2^(i) w_0^(j) w_1^(k) + w_2^(i) w_1^(j) w_0^(k)。当 l=3 时，c_α = Σ_{i,j,k∈Pα} w_0^(i) w_1^(j) w_2^(k) + w_0^(i) w_1^(j) w_3^(k) + w_0^(i) w_2^(j) w_1^(k) + w_0^(i) w_2^(j) w_3^(k) + w_0^(i) w_3^(j) w_1^(k) + w_0^(i) w_3^(j) w_2^(k) + w_1^(i) w_0^(j) w_2^(k) + w_1^(i) w_0^(j) w_3^(k) + w_1^(i) w_2^(j) w_0^(k) + w_1^(i) w_2^(j) w_3^(k) + w_1^(i) w_3^(j) w_0^(k) + w_1^(i) w_3^(j) w_2^(k) + w_2^(i) w_0^(j) w_1^(k) + w_2^(i) w_0^(j) w_3^(k) + w_2^(i) w_1^(j) w_0^(k) + w_2^(i) w_1^(j) w_3^(k) + w_2^(i) w_3^(j) w_0^(k) + w_2^(i) w_3^(j) w_1^(k) + w_3^(i) w_0^(j) w_1^(k) + w_3^(i) w_0^(j) w_2^(k) + w_3^(i) w_1^(j) w_0^(k) + w_3^(i) w_1^(j) w_2^(k) + w_3^(i) w_2^(j) w_0^(k) + w_3^(i) w_2^(j) w_1^(k)。
...
参数共享不仅使模型更高效，还使模型能够泛化到未见过的特征交互，并对噪声更加鲁棒。例如，考虑具有稀疏特征的数据集。如果两个二值特征 x_i 和 x_j 在训练数据中很少或从未同时出现，即 x_i ≠ 0 ∧ x_j ≠ 0，那么学习到的 x_i x_j 的权重将不携带任何有意义的预测信息。

FM 是一种浅层结构，仅限于表示 2 阶交叉项。相比之下，DCN 能够构建所有度数 |α| 受限于某个由层深度决定的常数的交叉项 x_1^α1 x_2^α2 ... x_d^αd，如定理 3.1 所述。因此，交叉网络将参数共享的思想从单层扩展到了多层和高阶交叉项。注意，与高阶 FM 不同，交叉网络中的参数数量仅随输入维度线性增长。

### 3.3 高效投影

每个交叉层以高效的方式将所有 x_0 和 x_l 之间的两两交互投影回输入的维度。

将 ~x ∈ R^d 视为交叉层的输入。交叉层首先隐式地构造了 d^2 个两两交互 x_i ~x_j，然后以内存高效的方式将它们隐式地投影回维度 d。然而，直接方法的代价是立方级的。

我们的交叉层提供了一种高效的解决方案，将代价降低到与维度 d 呈线性关系。考虑 x_p = x_0 ~x^T w。这实际上等价于

x^T_p = [x_1 ~x_1 ... x_1 ~x_d ... x_d ~x_1 ... x_d ~x_d] * diag_block(w),   (8)

其中行向量包含所有 d^2 个两两交互 x_i ~x_j，投影矩阵具有块对角结构，w ∈ R^d 是一个列向量。
...
**证明。** 符号。令 i 为一个由 0 和 1 组成的多重索引向量，其最后一个条目固定为 1。对于多重索引 α = [α_1, ..., α_d] ∈ N^d 和 x = [x_1, ..., x_d]^T，我们定义 |α| = Σ^d_{i=1} α_i，以及 x^α = x_1^α1 x_2^α2 ... x_d^αd。

我们首先通过归纳法证明：

g_l(x_0) = x^T_l w_l = Σ^{l+1}_{p=1} Σ_{|i|=p} Π^l_{j=0} (x^T_0 w_j)^{i_j},                    (9)
...
g_k(x_0) = x^T_k w_k = Σ^{k+1}_{p=1} Σ_{|i|=p} Π^k_{j=0} (x^T_0 w_j)^{i_j}。
...
x^T_{k+1} w_{k+1} = (Σ^{k+1}_{p=1} Σ_{|i|=p} Π^k_{j=0} (x^T_0 w_j)^{i_j}) (x^T_0 w_{k+1}) + Σ^{k+1}_{p=1} Σ_{|i|=p, i_k=0} Π^{k-1}_{j=0} (x^T_0 w_j)^{i_j} + (x^T_0 w_{k+1}) + Π^k_{j=0} (x^T_0 w_j)

= Σ^{k+2}_{p=2} Σ_{|i|=p, i_k=1} Π^{k+1}_{j=0} (x^T_0 w_j)^{i_j} + Σ^{k+1}_{p=1} Σ_{|i|=p, i_k=0} Π^{k+1}_{j=0} (x^T_0 w_j)^{i_j}

= Σ^{k+2}_{p=1} Σ_{|i|=p} Π^{k+1}_{j=0} (x^T_0 w_j)^{i_j}.                                     (11)

第一个等式是将 i 的大小从 k+1 增加到 k+2 的结果。第二个等式利用了 i 的最后一个条目总是 1 的定义，最后一个等式也应用了同样的规则。由归纳假设，公式 9 对所有 l ∈ Z 成立。

接下来，我们通过重排公式 9 中的项来计算 x^α 的系数 c_α(w_0, ..., w_l)。注意 x_1...x_1 ... x_d...x_d 的所有不同排列都形如 x^α。因此，c_α 是与公式 9 中出现的每个排列相关联的所有权重的总和。排列 x_{j1} x_{j2} ... x_{jp} 的权重为：

Σ_{i_1,...,i_p} w_{j_1}^{(i_1)} w_{j_2}^{(i_2)} ... w_{j_p}^{(i_p)},

其中 (i_1, ..., i_p) 属于所有对应活动索引的集合 |i|=p，具体地：

(i_1, ..., i_p) ∈ B_p = { y ∈ {0, 1, ..., l}^p | y_i < y_j ∧ y_p = l }.

因此，如果我们记 P_α 为 (1...1 ... d...d) 的所有排列的集合，那么我们就得到了我们的结论：
   |_α1_|     |_αd_|

c_α = Σ_{j_1,...,j_p∈P_p} Σ_{i_1,...,i_p∈B_p} Π^p_{k=1} w_{j_k}^{(i_k)}.                          (12)
