...
**特征表示** 在我们的在线展示系统中，我们使用四类特征：用户画像（User Profile）、用户行为（User Behavior）、广告（Ad）和上下文（Context）。值得注意的是，广告也是商品。为统一表述，本文中我们将广告称为目标商品。每类特征都有几个字段：用户画像的字段包括性别、年龄等；用户行为的字段是用户访问过的商品ID列表；广告的字段包括广告ID、店铺ID等；上下文的字段包括时间等。每个字段中的特征可以编码为独热向量，例如用户画像类别中的女性特征编码为[0, 1]。来自用户画像、用户行为、广告和上下文的不同字段的独热向量的拼接分别构成xp、xb、xa和xc。在序列CTR模型中，值得注意的是每个字段包含一个行为列表，每个行为对应一个独热向量，可以表示为xb = [b1; b2; ···; bT] ∈ R^(K×T)，bt ∈ {0,1}^K，其中bt编码为独热向量并表示第t个行为，T是用户历史行为的数量，K是用户可以点击的商品总数。

**BaseModel的结构** 大多数深度CTR模型建立在嵌入和MLP的基本结构上。基本结构由几个部分组成：

- **嵌入（Embedding）** 嵌入是将大规模稀疏特征转换为低维稠密特征的常见操作。在嵌入层中，每个特征字段对应一个嵌入矩阵，例如访问过的商品的嵌入矩阵可以表示为Egoods = [m1; m2; ···; mK] ∈ R^(nE×K)，其中mj ∈ R^nE表示维度为nE的嵌入向量。特别地，对于行为特征bt，如果bt[jt] = 1，则其对应的嵌入向量为mjt，一个用户的有序行为嵌入向量列表可以表示为eb = [mj1; mj2; ···, mjT]。类似地，ea表示广告类别中各字段拼接后的嵌入向量。
...
L_target = -(1/N) * Σ_{(x,y)∈D} (y log p(x) + (1-y) log(1-p(x)))   (1)

其中x = [xp, xa, xc, xb] ∈ D，D是大小为N的训练集。y ∈ {0,1}表示用户是否点击目标商品。p(x)是网络的输出，即用户点击目标商品的预测概率。
...
u_t = σ(W^u i_t + U^u h_{t-1} + b^u)       (2)
r_t = σ(W^r i_t + U^r h_{t-1} + b^r)       (3)
ĥ_t = tanh(W^h i_t + r_t ∘ U^h h_{t-1} + b^h)   (4)
h_t = (1 - u_t) ∘ h_{t-1} + u_t ∘ ĥ_t       (5)

其中σ是sigmoid激活函数，∘是逐元素乘积，W^u, W^r, W^h ∈ R^(nH×nI)，U^z, U^r, U^h ∈ R^(nH×nH)，nH是隐藏层大小，nI是输入大小。i_t是GRU的输入，i_t = eb[t]表示用户执行的t-th行为，h_t是第t个隐藏状态。

然而，仅捕获行为间依赖关系的隐藏状态h_t无法有效表示兴趣。由于目标商品的点击行为是由最终兴趣触发的，L_target中使用的标签只包含监督最终兴趣预测的真实值，而历史状态h_t（t < T）无法获得适当的监督。众所周知，每一步的兴趣状态直接导致连续行为。因此我们提出了辅助损失，利用行为b_{t+1}来监督兴趣状态h_t的学习。除了使用真实的下一个行为作为正样本外，我们还使用从商品集中除点击商品外采样的负样本。共有N对行为嵌入序列：{e^i_b} ∈ DB, i ∈ 1,2,···,N，其中e^i_b ∈ R^(T×nE)表示点击的行为序列，ê^i_b ∈ R^(T×nE)表示负样本序列。T是历史行为数量，nE是嵌入维度，e^i_b[t] ∈ G表示用户i点击的第t个商品的嵌入向量，G是全部商品集合。ê^i_b[t]表示从商品集中除用户在t步点击的商品外采样的商品的嵌入向量。辅助损失可以表述为：

L_aux = -(1/N) * Σ_{i=1}^{N} Σ_t ( log σ(h^i_t, e^i_b[t+1]) + log(1 - σ(h^i_t, ê^i_b[t+1])) )   (6)

其中σ(x1, x2) = 1/(1+exp(-[x1,x2]))是sigmoid激活函数，h^i_t表示用户i的GRU的第t个隐藏状态。我们在CTR模型中使用的全局损失为：

L = L_target + α * L_aux   (7)
...
a_t = exp(h_t W e_a) / Σ_{j=1}^{T} exp(h_j W e_a)   (8)

其中e_a是广告类别中各字段嵌入向量的拼接，W ∈ R^(nH×nA)，nH是隐藏状态的维度，nA是广告嵌入向量的维度。注意力分数可以反映广告e_a和输入h_t之间的关系，强相关性导致大的注意力分数。
...
- **带有注意力更新门的GRU（AUGRU）** 尽管AGRU可以直接使用注意力分数控制隐藏状态的更新，但它使用标量（注意力分数a_t）替换向量（更新门u_t），忽略了不同维度之间重要性的差异。我们提出了带有注意力更新门的GRU（AUGRU）来无缝地结合注意力机制和GRU：
  ...
  ũ'_t = a_t * u'_t   (11)
  h'_t = (1 - ũ'_t) ∘ h'_{t-1} + ũ'_t ∘ ĥ'_t   (12)
