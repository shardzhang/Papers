...
> Umar Farooq, M.H. Maqbool, Adib Mosharrof, A.B. Siddique, Hassan Foroosh

---

本文介绍了 MobileRec: A Large-Scale Dataset for Mobile Apps Recommendation。核心内容：
...
HR@K = (1/M) * \Sigma_{u=1}^{M} WR, where WR = \Sigma_{R=1}^{N} WR · 1{R_u \leq K} · 1{R_u = R} · (1/M) · \Sigma_{u=1}^{M} 1X

其中 1X 表示示性随机变量，M 是用户总数，N = |I| 是总item数。R 是item在范围 [1, N] 内的整数排名位置。R_u 是item i_u 在 I 个item中对用户 u 的排名。WR 捕捉在位置 R 上拥有item i_u 的用户。

NDCG@K 可表示为[22, 49]：

(1/|U|) * \Sigma_{u\inU} (\Sigma_{i=1}^{K} \delta(i \in R(u)) / log2(i+1)) / (\Sigma_{i=1}^{min(|R(u)|,K)} 1/log2(i+1))
