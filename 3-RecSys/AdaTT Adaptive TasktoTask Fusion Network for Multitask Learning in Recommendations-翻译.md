...
下面我们详细说明公式3中引入的FU_t^l的构建。对于任务t，在接收来自前一融合层级的所有输出后，我们首先为该任务构建mt个原生专家，表示为E_{t,i}^l，使用函数e_{t,i}^l(·)和输入f^{l-1}(·)。即：

E_{t,i}^l(f^{l-1}_t(x)) = e_{t,i}^l(f^{l-1}_t(x)),         (4)

其中i = 1, 2, ..., mt，且E_{t,i}^l ∈ R^{1×dl}。第l层的每个专家网络产生一个长度为dl的向量。为方便表示，在第l层，我们使用E_t^l和E^l分别表示属于任务t的专家和所有任务的所有专家的垂直拼接。具体来说，E_t^l和E^l表示为：

E_t^l = [E_{t,1}^l, E_{t,2}^l, ..., E_{t,mt}^l],             (5)
E^l = [E_1^l, E_2^l, ..., E_T^l],                           (6)

其中E_t^l ∈ R^{mt×dl}且E^l ∈ R^{(m1+m2+...+mT)×dl}。在上述等式中，[,]表示将向量或子矩阵垂直堆叠成更大矩阵的操作。
...
其中E^l通过门控G_t^l相乘，G_t^l ∈ R^{(m1+m2+...+mT)×1}由函数g_t^l(·)生成，而E_t^l通过一个学习的向量v_t^l ∈ R^{mt×1}在NativeExpertLF中简单地组合。当m1 = m2 = ... = mT = 1时，即所有融合单元只有一个专家，NativeExpertLF_t^l(E_t^l)退化为E_t^l，即为原生专家分配单位权重以简化。对于g_t^l有许多设计选项。一种常见的是使用由softmax激活的单层MLP：

g_t^l(f_t^{l-1}(x)) = softmax(W_t^l f_t^{l-1}(x)),            (10)

这里W_t^l ∈ R^{(m1+m2+...+mT)×d_{l-1}}是一个学习到的矩阵。
