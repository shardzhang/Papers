...
> Xiaoyong Yang, Yadong Zhu, Yi Zhang, Xiaobo Wang, Quan Yuan | Alibaba Inc, Beijing, China; UC Santa Cruz, USA

本文提出了Swing与Surprise两种新算法，用于在电子商务中大规模构建商品图：Swing算法利用用户-item点击二部图的内部结构（swing，摆动）捕捉商品间的替代关系，Surprise算法利用商品类别信息与聚类技术建模互补关系。核心内容：

- **Swing替代关系建模**：利用用户-item行为图的准局部内部结构计算商品间相似度，比传统CF方法使用的单条边更稳定，可减少噪声用户行为数据的影响
- **Surprise互补关系建模**：利用商品类别信息和基于Swing算法的商品聚类解决用户共同购买图的稀疏性问题，并考虑共同购买的时间敏感性和时间顺序
- **大规模并行实现**：两种方法均基于Map-Reduce或Spark等大规模分布式计算框架并行实现，可在淘宝构建商品替代图和互补图，为推荐排序模块提供基础索引服务
- **实验验证**：离线实验中在精确率、召回率和MAP指标上显著优于基线CF方法，在线实验中CTR、CVR和PPM指标均显著提升

关键发现：Swing利用用户-item二部图的内部结构而非单条边，提供了更可靠的计算传播并消除了噪声数据的影响；Surprise通过商品类别信息与聚类技术有效解决了用户共同购买图的稀疏性问题，两者均可高效扩展至数十亿商品的规模。
...
- **信息系统 $\rightarrow$ 数据库应用；数据挖掘。**

## 关键词

Product Graph, E-commerce, Recommendation system
...
随着数十亿用户利用电子商务网站来满足购物需求，同时节省时间并避免拥挤，如何更好地服务在线客户正成为一个日益重要的问题。同时，像淘宝和亚马逊这样的大型电子商务网站通常提供数十亿种商品供消费者选择。推荐系统可以帮助消费者选择合适的商品以满足其品味和特殊需求，在现代电子商务中扮演着关键角色[10]。现有的推荐方法主要分为两类：协同过滤方法（CF，Collaborative Filtering）[14, 24]和基于内容的方法（CB，Content-Based）[15, 17, 18]。还存在其他方法，如结合两者的混合方法，或考虑用户当前上下文信息以使推荐更合理的上下文感知方法[2, 1]。
...
为了评估我们方法的有效性，我们进行了广泛的离线数据实验和在线用户研究。在大型数据集上的离线实验结果表明，所提出的方法在精确率、召回率和平均精度均值（MAP，Mean Average Precision）指标上显著优于现有经过良好调优的基线 CF 方法。在淘宝上使用真实电子商务用户进行的在线实验结果也表明，所提出的方法能够带来显著更高的点击率（CTR，Click-Through Rate）、转化率（CVR，Conversion Rate）和千次展示收入（PPM，Payment Per Thousand Impressions）。本文还分析和展示了新方法在离线运行时间方面的效率。
...
其中 $r_{u,i}$ 是用户 $u$ 对item $i$ 的评分， $\bar{r}_i$ 是item $i$ 的平均评分。item $i$ 和item $j$ 之间的相似度通过计算 Pearson 系数来衡量。
...
在这种情况下，我们有 $|U_h| = 5$ ， $|U_t| = 15$ ， $|U_p| = 40$ ， $|U_q| = 60$ ， $|U_z| = 4$ ，等等。这意味着总共有 5 个用户点击了商品 h，15 个用户点击了商品 t，等等。如果使用余弦相似度对 h 的邻近item进行排序，结果将是 t > z > p > q。然而，我们可以看到 z 与 h 并不是非常相似。除了共同邻居的数量之外，它只取决于分母上每个item节点的度（即有多少用户点击了每个item），这可能受到许多因素的影响，具有很大的不确定性。注意 z 的 CF 得分可能大于 p 和 q，因为它不那么流行。
...
**算法1：用于替代关系的 Swing 算法**

$$
\begin{aligned}
&\textbf{输入：}\ \text{用户和item索引表}\ U,\ I;\ \text{平滑系数}\ \alpha \\
&\textbf{输出：}\ \text{item}\ item_i\ \text{的替代item列表} \\
&\textbf{for each}\ u \in U_i\ \textbf{do} \\
&\quad w_u \leftarrow \frac{1}{\sqrt{|I_u|}} \\
&\quad \textbf{for each}\ v \in U_i \setminus \{u\}\ \textbf{do} \\
&\quad\quad w_v \leftarrow \frac{1}{\sqrt{|I_v|}} \\
&\quad\quad k \leftarrow |I_u \cap I_v| \\
&\quad\quad \textbf{for each}\ j \in I_u \cap I_v\ \textbf{do} \\
&\quad\quad\quad Swing[j] \mathrel{+}= w_u \cdot w_v \cdot \frac{1}{\alpha + k} \\
&\quad\quad \textbf{end for} \\
&\quad \textbf{end for} \\
&\textbf{end for} \\
&\textbf{return}\ Swing_i = (Swing_i[1], \cdots, Swing_i[n]) \\
\end{aligned}
$$
...
$$
s_1(i,j) = \sum_{u \in U_i \cap U_j} \frac{1/(1 + |t_{ui} - t_{uj}|)}{\|U_i\| \times \|U_j\|} \qquad (5)
$$
...
$$
\begin{aligned}
&\textbf{输入：}\ \text{相似度图}\ G(V, E),\ \text{item节点}\ x \in V\ \text{及其邻居}\ \Gamma(x),\ \text{阻尼因子}\ \beta = 0.25 \\
&\textbf{输出：}\ \text{唯一标签}\ L(x) \\
&\textbf{init}\ L(x) \leftarrow x \\
&\textbf{for}\ t = 1, \ldots, n\ \textbf{do} \\
&\quad \textbf{for each}\ x \in V\ \textbf{do} \\
&\quad\quad \textbf{init}\ p[L[y]] \leftarrow 0,\ y \in \Gamma(x) \\
&\quad\quad \textbf{for each}\ y \in \Gamma(x)\ \textbf{do} \\
&\quad\quad\quad p[L[y]] \mathrel{+}= e_{y,x} \\
&\quad\quad \textbf{end for} \\
&\quad\quad \textbf{if}\ random() > \beta\ \textbf{then} \\
&\quad\quad\quad \textbf{Set}\ L(x) \leftarrow k,\ \text{其中}\ p[k] = \max(p[1:m]) \\
...
$$
w_{i,j} = \frac{\sum_{u \in U_i \cap U_j} w_u^2}{\sqrt{\sum_{u \in U_i} w_u^2} \times \sqrt{\sum_{v \in U_j} w_v^2}}
$$
...
³数据集将在我们的官方网站上稍后发布：https://tianchi.shuju.aliyun.com/datalab/index.htm
