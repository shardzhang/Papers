...
> Ruoxi Wang, Zhe Zhao, Xinyang Yi, Ji Yang, Derek Zhiyuan Cheng, Lichan Hong, Steve Tjoa, Jieqi Kang, Evan Ettinger, Ed H. Chi  
> Google Inc.  
...
机器学习的大规模检索系统需要大量训练数据来表示查询-物品相关性。然而，收集用户的显式反馈成本高昂。在本文中，我们提出利用用户日志和隐式反馈作为辅助目标来改善检索系统中的相关性建模。具体来说，我们采用双塔神经网络架构，结合协同信息和内容信息来建模查询-物品相关性。通过引入使用更丰富的隐式用户反馈数据训练的辅助任务，我们提高了查询和物品学习表示的质量和区分度。将这些学习到的表示应用到工业检索系统中，已经带来了显著的改进。
...
- **高效的检索算法。** 在给定学习到的表示后，研究者提出了高效算法，根据与嵌入空间相关联的相似性（距离）度量来检索 top-k 相关项目（Broder, 1997；Guo 等人，2016）。

然而，设计和开发实际的大规模检索系统面临诸多挑战：

- **稀疏的相关性数据。** 收集用户对物品相关性的真实意见成本高昂。通常，研究人员和工程师设计带有 Likert 量表问题的评估模板来评价相关性（Chang 等人，2015），并通过众包平台（如 Amazon Mechanical Turk）征集反馈。
...
现有研究也涉及基于张量分解的上下文感知检索应用中的多任务检索系统（Zhao 等人，2015）。不幸的是，由于模型容量和服务时间限制的约束，该模型难以轻松适应从多个特征源学习复杂的特征表示。许多基于多任务 DNN 的推荐系统（Covington 等人，2016；Ma 等人，2018）是为排序问题设计的，其中仅对少量高质量候选进行评分。这些完整的排序解决方案难以轻易应用于检索问题，在检索问题中，我们需要从拥有数百万到数亿候选项目的大规模语料库中识别出数千个候选。
...
检索问题定义如下：给定一个查询和一个候选项集语料库，返回 top-k 最相关的项目。设 {xᵢ}ⁿᵢ₌₁ ⊆ X 和 {yⱼ}ᴹⱼ₌₁ ⊆ Y 分别为特征空间 X 和 Y 中查询和候选项的特征向量，其中 N 和 M 分别表示查询和候选项的数量。我们将检索系统建模为一个参数化评分函数 s(·, ·; θ) : X × Y → R，其中 θ 表示模型参数。在推理时，对于给定的查询，选择具有 top-k 得分 s(x, y; θ) 的项目。我们假设训练数据是一组查询-项目对 {(xₜ, yₜ)}ᵀₜ₌₁，其中 yₜ 是与 xₜ 关联的候选项，具有显式或隐式的用户反馈，在实际中 T << MN。我们的目标是基于这些 T 个样本拟合评分函数。

### 3.2 使用用户反馈进行训练

在训练基于机器学习的检索系统时，理想的方法是使用用户的显式反馈，这反映了项目与查询的相关性。然而，获取用户的显式反馈成本高昂；因此，许多现有系统使用来自用户日志的隐式反馈，如点击。
...
为了促进表示学习，我们引入了一个辅助目标，用于捕获用户对项目的参与度，例如项目点击、购物检索中的产品购买、或电影推荐中的电影观看。

形式上，我们旨在同时学习两个目标 sₑₓₚ(·, ·; θ) 和 sᵢₘₚ(·, ·; θ′)，同时共享 θ 和 θ′ 之间的部分参数。我们假设一些样本 (xₜ, yₜ) 在集合 E 中带有显式反馈，而其他样本在集合 I 中带有隐式反馈。此外，每个样本 (xₜ, yₜ) ∈ E 关联一个标签 lₜ ∈ R，表示用户的显式反馈，例如对相关性调查的响应。注意 E 和 I 并非互斥，因为一些样本可以同时具有隐式和显式反馈。

我们使用回归损失来拟合集合 E 中样本的用户显式反馈。一种示例损失是均方误差（MSE）：

Lₑₓₚ(θ; E) = (1/|E|) Σ_(xₜ,yₜ)∈E (sₑₓₚ(xₜ, yₜ; θ) − lₜ)²   (1)

另一方面，我们将隐式反馈的建模视为对整个项目语料库的多类分类任务，并使用 softmax 公式来建模选择项目 y 的概率，即：

P(y | x; θ′) = exp(sᵢₘₚ(x, y; θ′)) / Σⱼ₌₁ᴹ exp(sᵢₘₚ(x, yⱼ; θ′))

极大似然估计（MLE）可表述为：

Lᵢₘₚ(θ′; I) = −(1/|I|) Σ_(xₜ,yₜ)∈I log(P(yₜ | xₜ; θ′))   (2)

使用损失权重 w 和 w′，我们通过联合优化公式 (1) 和 (2) 中的损失：

L(θ, θ′) = w·Lₑₓₚ(θ; E) + w′·Lᵢₘₚ(θ′; I)
...
图 1 提供了双塔 DNN 模型架构的高级图示。给定一对由特征向量 x ∈ X、y ∈ Y 表示的查询和项目，左侧和右侧塔分别提供两个基于 DNN 的参数化嵌入函数 u : X × Rᵈ → Rᵏ、v : Y × Rᵈ → Rᵏ，它们将查询和项目的特征编码到 k 维嵌入空间。评分函数然后计算为查询嵌入和项目嵌入在顶层的点积，即：
...
在本节中，我们描述在 Google 的一个大规模检索系统（用于相关项目推荐，例如应用推荐）上对所提出框架进行的实验。
