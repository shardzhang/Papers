# 多门混合专家多任务学习中的任务关系建模（MMoE）

> Jiaqi Ma, Zhe Zhao, Xinyang Yi, Jilin Chen, Lichan Hong, Ed H. Chi | Google

本文提出 MMoE（Multi-gate Mixture-of-Experts），**将 MoE 结构适配到多任务学习中，通过共享专家子模型 + 每个任务独立门控网络来显式建模任务关系**。核心发现是——**任务相关性越低，MMoE 相对基线的优势越明显，且门控机制带来额外的可训练性收益**。

核心内容：

- 传统共享底层（Shared-Bottom）多任务模型在任务不相关时精度显著退化，因为不同任务的归纳偏置产生冲突
- 提出 MMoE：将共享底层拆分为一组专家网络，每个任务有独立的门控网络，学习不同的专家组合权重来捕获任务关系
- 相比 Cross-stitch 等软参数共享方法，MMoE 不需要为每个任务添加大量额外参数，计算开销可控
- 在合成数据上验证：任务相关性低时 MMoE 优势明显；在真实推荐系统上验证：AUC 显著提升

关键发现：

- 合成数据实验：Pearson 相关性从 0.5 降到 -1 时，MMoE 相对 Shared-Bottom 的 Macro-AUC 提升从 +0.3% 扩大到 +1.2%
- 可训练性：MMoE 在多次随机初始化下收敛更稳定，标准差比 Shared-Bottom 低约 40%
- Census-income 数据集：MMoE 在两个二分类任务上均优于所有基线（Cross-stitch、Deep Relationship 等）
- 大规模推荐系统：MMoE 离线 AUC 提升 +0.2%~+0.5%，在线指标持续正向
...
**关键词：** multi-task learning；mixture-of-experts；neural networks；recommender systems
...
MMoE的主干建立在最常用的共享底层多任务DNN结构[8]之上。

**图1：多任务模型。(a) Shared-Bottom 模型。(b) 单门 MoE 模型。(c) 多门 MoE 模型。**

![图1](/Users/dazhang/PycharmProject/Papers/3-RecSys/.picture/2018-MMoE-Modeling Task Relationships in Multi-task Learning with Multi-gate Mixture-of-Experts-fig1.png)

共享底层模型结构如图1(a)所示，其中输入层之后的几个底层在所有任务之间共享，然后每个任务在底层表示之上有一个单独的"塔"网络。与所有任务共享一个底层网络不同，我们的模型（如图1(c)所示）有一组底层网络，每个网络被称为一个专家。在我们的论文中，每个专家是一个前馈网络。然后我们为每个任务引入一个门控网络。门控网络接收输入特征并输出softmax门，以不同的权重组合专家，允许不同的任务以不同的方式利用专家。组合专家的结果然后被传递到任务特定的塔网络。通过这种方式，不同任务的门控网络可以学习不同的专家组合混合模式，从而捕获任务关系。
...
给定 $K$ 个任务，该模型由一个表示为函数 $f$ 的共享底层网络和 $K$ 个塔网络 $h_k$ 组成，其中 $k = 1, 2, \ldots, K$ 分别对应每个任务。共享底层网络位于输入层之后，塔网络构建在共享底层的输出之上。然后每个任务的单独输出 $y_k$ 跟随对应的任务特定塔。对于任务 $k$，模型可以表述为：
...
(1) 给定输入特征维度 $d$，我们生成两个正交单位向量 $u_1, u_2 \in \mathbb{R}^d$，即 $u_1^\top u_2 = 0$，$\|u_1\|_2 = 1$，$\|u_2\|_2 = 1$。

(2) 给定一个比例常数 $c$ 和一个相关性分数 $-1 \leq p \leq 1$，生成两个权重向量 $w_1, w_2$ 使得：
$$
w_1 = c u_1, \quad w_2 = c(p u_1 + \sqrt{1-p^2} u_2) \qquad (2)
$$

(3) 随机采样一个输入数据点 $x \in \mathbb{R}^d$，其每个元素来自 $\mathcal{N}(0, 1)$。

(4) 为两个回归任务生成两个标签 $y_1, y_2$ 如下：
$$
y_1 = w_1^\top x + \sum_{i=1}^{m} \sin(\alpha_i w_1^\top x + \beta_i) + \epsilon_1 \qquad (3)
$$
$$
y_2 = w_2^\top x + \sum_{i=1}^{m} \sin(\alpha_i w_2^\top x + \beta_i) + \epsilon_2 \qquad (4)
$$
其中 $\alpha_i, \beta_i, i = 1, 2, \ldots, m$ 是控制正弦函数形状的给定参数，$\epsilon_1, \epsilon_2 \sim \mathcal{N}(0, 0.01)$。

(5) 重复步骤(3)和(4)直到生成足够的数据。

由于非线性数据生成过程，直接生成具有给定标签Pearson相关性的任务并不简单。相反，我们操作公式2中权重向量的余弦相似度，即 $\cos(w_1, w_2) = p$，然后测量结果标签的Pearson相关性。注意在线性情况下，$y_1 = w_1^\top x + \epsilon_1$，$y_2 = w_2^\top x + \epsilon_2$，$y_1, y_2$ 的标签Pearson相关性正好是 $p$。在非线性情况下，公式3和公式4中的 $y_1$ 和 $y_2$ 也是正相关的，如图2所示。在本文的其余部分，为简单起见，我们将权重向量的余弦相似度称为"任务相关性"。

![图2](/Users/dazhang/PycharmProject/Papers/3-RecSys/.picture/2018-MMoE-Modeling Task Relationships in Multi-task Learning with Multi-gate Mixture-of-Experts-fig2.png)
...
其中 $\sum_{i=1}^{n} g(x)_i = 1$，$g(x)_i$ 是 $g(x)$ 输出的第 $i$ 个logit，表示专家 $f_i$ 的概率。

这里，$f_i, i = 1, \ldots, n$ 是 $n$ 个专家网络，$g$ 表示一个门控网络，它组合所有专家的结果。更具体地说，门控网络 $g$ 基于输入产生 $n$ 个专家的分布，最终输出是所有专家输出的加权和。

![图3](/Users/dazhang/PycharmProject/Papers/3-RecSys/.picture/2018-MMoE-Modeling Task Relationships in Multi-task Learning with Multi-gate Mixture-of-Experts-fig3.png)
...
![图4](/Users/dazhang/PycharmProject/Papers/3-RecSys/.picture/2018-MMoE-Modeling Task Relationships in Multi-task Learning with Multi-gate Mixture-of-Experts-fig4.png)

图4：MMoE、OMoE和共享底层在具有不同相关性的合成数据上的平均性能。

### 4.2 多门混合专家

我们提出了一种新的MoE模型，旨在捕获任务差异，而不需要比共享底层多任务模型显著更多的模型参数。这个新模型被称为多门混合专家（MMoE）模型，其关键思想是用公式5中的MoE层替代公式1中的共享底层网络 $f$。更重要的是，我们为每个任务 $k$ 添加一个单独的门控网络 $g^k$。更确切地说，任务 $k$ 的输出为：
...
其中：
$$
f^k(x) = \sum_{i=1}^{n} g^k(x)_i f_i(x) \qquad (7)
$$
...
$$
g^k(x) = \text{softmax}(W_g^k x) \qquad (8)
$$

其中 $W_g^k \in \mathbb{R}^{n \times d}$ 是一个可训练的矩阵，$n$ 是专家数量，$d$ 是特征维度。
...
**模型结构：** 输入维度为100。两种基于MoE的模型都有8个专家，每个专家实现为单层网络。专家网络中隐藏层的大小为16。塔网络仍然是大小为8的单层网络。我们注意到，共享专家和塔中的模型参数总数为 $100 \times 16 \times 8 + 16 \times 8 \times 2 = 13056$。对于基线共享底层模型，我们仍然将塔网络设置为大小为8的单层网络。我们将单层共享底层网络的大小设置为 $13056/(100 + 8 \times 2) \approx 113$。
...
![图5](/Users/dazhang/PycharmProject/Papers/3-RecSys/.picture/2018-MMoE-Modeling Task Relationships in Multi-task Learning with Multi-gate Mixture-of-Experts-fig5.png)
...
给定 $y^k$ 作为任务 $k$（$k \in \{1,2\}$）的真实标签，任务 $k$ 的预测表示为 $\hat{y}^k = f(x; \theta^k)$，其中 $\theta^k$ 是模型参数。

该方法的损失函数为：$L(y^1, f(x; \theta^1)) + L(y^2, f(x; \theta^2)) + \alpha\|\theta^1 - \theta^2\|_2^2$，其中 $y^1, y^2$ 是任务1和任务2的真实标签，$\alpha$ 是一个超参数。该方法通过 $\alpha$ 的大小来建模任务相关性。

**Cross-Stitch[27]：** 该方法通过引入"Cross-Stitch"单元在两个任务之间共享知识。Cross-Stitch单元接收来自任务1和2的分离隐藏层的输入 $x^1$ 和 $x^2$，并通过以下方程输出 $\tilde{x}^1$ 和 $\tilde{x}^2$：

$$
[\tilde{x}^1; \tilde{x}^2] = \begin{bmatrix} \alpha_{11} & \alpha_{12} \\ \alpha_{21} & \alpha_{22} \end{bmatrix} [x^1; x^2]
$$

其中 $\alpha_{jk}, j,k=1,2$ 是可训练参数，表示从任务 $k$ 到任务 $j$ 的交叉迁移。$\tilde{x}^1$ 和 $\tilde{x}^2$ 分别被发送到任务1和任务2的更高级别层。

**张量分解[34]：** 在该方法中，来自多个任务的权重被建模为张量，并使用张量分解方法进行跨任务参数共享。在我们的比较中，我们实现了用于学习多任务模型的Tucker分解，据报道它提供了最可靠的结果[34]。例如，给定输入隐藏层大小 $m$、输出隐藏层大小 $n$ 和任务数量 $k$，权重 $W$（一个 $m \times n \times k$ 张量）由以下方程得到：

$$
W = \sum_{i_1} \sum_{i_2} \sum_{i_3} S(i_1, i_2, i_3) \cdot U_1(:, i_1) \circ U_2(:, i_2) \circ U_3(:, i_3)
$$

其中大小为 $r_1 \times r_2 \times r_3$ 的张量 $S$、大小为 $m \times r_1$ 的矩阵 $U_1$、大小为 $n \times r_2$ 的矩阵 $U_2$ 和大小为 $k \times r_3$ 的矩阵 $U_3$ 是可训练参数。它们都通过标准反向传播一起训练。$r_1, r_2, r_3$ 是超参数。
...
- 张量分解：Tucker分解的 $r_1, r_2, r_3$，隐藏层大小。
...
![图6](/Users/dazhang/PycharmProject/Papers/3-RecSys/.picture/2018-MMoE-Modeling Task Relationships in Multi-task Learning with Multi-gate Mixture-of-Experts-fig6.png)
