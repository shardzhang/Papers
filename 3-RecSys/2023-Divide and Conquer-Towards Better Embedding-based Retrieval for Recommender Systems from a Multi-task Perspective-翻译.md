# 分而治之：从多任务视角迈向更好的基于嵌入的推荐系统检索

> Yuan Zhang, Xue Dong∗, Weijie Ding, Biao Li, Peng Jiang†, Kun Gai | Kuaishou Technology, Shandong University
>
> WWW '23: The ACM Web Conference, April 30 - May 4, 2023, Austin, Texas, USA
>
> ∗ 在快手实习期间完成的工作。† 通讯作者。

本文介绍了分而治之：从多任务视角迈向更好的基于嵌入的推荐系统检索。核心内容：

- 提出分而治之方法：将整个候选集划分为多个语义相关的聚类，在每个聚类上并行运行基于嵌入的检索，再通过可控的合并策略组合各聚类结果，使检索模型只需专注于区分正样本与困难负样本
- 从多任务学习视角看待聚类内检索问题，借鉴基于提示的轻量级微调思想，提出高效的提示式任务适应技术，以极小的额外参数和计算开销提升每个聚类内的检索性能
- 该方法已在快手大规模推荐系统中部署超过四个月，并在公开数据集上进行了广泛的实验验证

关键发现：

- 在ML-1M和KuaiRand数据集上，相比基础模型SASRec分别取得高达27.9%和42.7%的召回率提升
- 在线A/B实验显示，该方法显著改善了包括App使用时长在内的关键用户参与度指标
- 提示式多任务学习在几乎不增加训练成本的前提下，优于MMoE等最先进的多任务学习方法
...
recommender systems, embedding-based retrieval, multi-task learning
...
值得注意的是，所提出的分而治之方法自然地引出了多任务学习（MTL，Multi-task Learning）场景[3]。从每个聚类中检索相关候选可以被视为一个独立的任务。这种多任务视角为从多任务学习角度优化模型精度开辟了新的方向。然而，在实际应用中，我们发现最先进的多任务学习方法（如MMoE（Multi-gate Mixture-of-Experts，多门控专家混合）[18]、PLE（Progressive Layered Extraction，渐进分层提取）[21]）显著增加了计算成本（尤其是在训练阶段），但并未达到预期的性能提升。在本文中，我们从近期基于提示的轻量级微调研究[12, 14]中汲取灵感，通过引入极少的额外参数和可忽略的计算开销，在输入层促进任务适应。由此产生的方法能够在统一的模型架构下对每个任务进行更高效的微调，并实现显著提升的精度。借助这一多任务学习范式，我们将基于嵌入的检索的潜力进一步推向深入。
...
**图1：动机。**(a) 以不同比例的困难负样本（与正样本属于同一聚类的负样本）与简单负样本训练EBR模型时的检索性能。"Within Cluster"和"Out of Cluster"分别表示当标准答案item与来自同一聚类的所有item混合以及与来自其他聚类的item混合时，检索到标准答案item的性能。(b) 用户兴趣的多模态结构。

## 2 方法

### 2.1 背景：基于嵌入的检索

假设我们有一个用户集 $U$ 和一个item集 $I$。我们将用户侧特征表示为每个用户 $u \in U$ 的 $\mathbf{x}_u$，将item侧特征表示为每个item $i \in I$ 的 $\mathbf{x}_i$。基于嵌入的检索方法使用用户编码器 $f$ 和item编码器 $g$ 将用户和item特征转换为嵌入 $\mathbf{e}_u = f(\mathbf{x}_u)$ 和 $\mathbf{e}_i = g(\mathbf{x}_i)$。用户嵌入 $\mathbf{e}_u$ 与item嵌入 $\mathbf{e}_i$ 之间的距离（最常见的是内积）用于表示item $i$ 对用户 $u$ 的相关性，记为 $r_{ui}$。

在部署期间，item嵌入 $\{\mathbf{e}_i\}_{i \in I}$ 在离线或近线系统中预先计算，并由近似最近邻搜索系统（如FAISS[10]）建立索引。因此，只有用户编码器 $\mathbf{e}_u = f(\mathbf{x}_u)$ 需要实时计算，近似最近邻系统可以高效地以亚线性时间检索最近邻item。

在本文中，我们以SASRec（Self-Attentive Sequential Recommendation，自注意力序列推荐）[11]为例介绍所提出的解决方案。SASRec使用Transformer[22]作为用户编码器：

$$
\mathbf{e}_u = f(\mathbf{x}_u) = \mathrm{Transformer}([\mathbf{s}_1^u, \mathbf{s}_2^u, \ldots, \mathbf{s}_n^u]) \qquad (1)
$$

其中 $\mathbf{s}_1^u, \mathbf{s}_2^u, \ldots, \mathbf{s}_n^u$ 是用户 $u$ 最近点击的 $n$ 个item的嵌入，并使用latent item ID嵌入单独作为item表示。

在训练过程中，item相关分数被训练用于区分用户点击的item（即 $I_u$）和其余item（即 $I \setminus I_u$）。例如，SASRec使用二元交叉熵损失来训练相关性模型：

$$
L = -\sum_{u \in U} \sum_{i^+ \in I_u} \left[ \log(\sigma(r_{ui^+})) + \mathbb{E}_{i^- \sim I \setminus I_u} \left[ \log(1 - \sigma(r_{ui^-})) \right] \right] \qquad (2)
$$

### 2.2 提出的分而治之方法

如上文引言所述，负样本的结构 $I \setminus I_u$ 可能非常多样化，其中一些明显不相关，而另一些可能相关但与某些其他项相比不具备足够的竞争力。随着候选集 $I$ 规模的增大，这个问题变得更加突出。

我们的解决方案源于"分而治之"的经典思想。我们首先将整个候选集划分为 $K$ 个语义相关的聚类 $I = \{C_1, C_2, \ldots, C_K\}$，然后利用基于嵌入的检索模型在每个聚类内而不是整个候选集中检索相关项（见图2）。需要注意的是，聚类标准可以根据不同的应用场景选择。我们的离线实验表明，使用Word2Vec[19]训练的item嵌入进行K-means[17]聚类是一个不错的默认选择。在我们的生产系统中，出于冷启动和可解释性的考虑，我们使用由内容特征（标题、脚本和图像等）预测的内部视频类别（如体育、美食、儿童等）。
...
$$
L = -\sum_{k=1}^{K} \sum_{u \in U} \sum_{i^+ \in I_u \cap C_k} \left[ \log(\sigma(r_{ui^+})) + \mathbb{E}_{i^- \sim C_k \setminus I_u} \left[ \log(1 - \sigma(r_{ui^-})) \right] \right] \qquad (3)
$$

一旦从每个聚类中获得了得分最高的item，我们可以为每个聚类分配适当的配额，以满足最终检索结果的自定义要求。例如，假设我们想要生成一个大小为 $M$ 的最终候选集。我们可以使用与用户编码器相同的特征训练一个用户意图模型，预测相关项落入聚类 $C_k$ 的概率 $p_{uk}$。然后，我们从每个聚类中合并 top-$M_k$ 个项作为最终结果，其中 $M_k = M \cdot \left[ (p_{uk})^\alpha / \sum_{k'=1}^{K} (p_{uk^{\prime}})^\alpha \right]$，$\alpha$ 是一个可调节的超参数^2。由于基于嵌入的检索可以在不同的item聚类上并行运行，且每次运行的搜索空间大约缩小了 $K$ 倍，这种分而治之的过程不会显著增加响应延迟。
...
$$
\mathbf{e}_u = f(\mathbf{x}_u) = \mathrm{Transformer}([\mathbf{t}_k; \mathbf{s}_1^u, \mathbf{s}_2^u, \ldots, \mathbf{s}_n^u]) \qquad (4)
$$

其中 $\mathbf{t}_k$ 是第 $k$ 个任务的可训练嵌入。然而，尽管这种方法减少了训练时间，性能仍未得到显著提升。

我们假设原因在于推荐模型中使用的Transformer层数远小于大多数自然语言处理场景^3。因此，仅通过几层自注意力块无法充分捕捉提示与原始输入之间的交互。于是，我们提出通过哈达玛积（Hadamard product）来施加显式的特征交互：

$$
\mathbf{e}_u = f(\mathbf{x}_u) = \mathrm{Transformer}([\mathbf{s}_1^u \odot \mathbf{t}_k, \mathbf{s}_2^u \odot \mathbf{t}_k, \ldots, \mathbf{s}_n^u \odot \mathbf{t}_k]) \qquad (5)
$$
...
- **ML-1M（MovieLens 1M）[5]** 包含由6,040名MovieLens用户对三千多部电影进行的一百万条匿名评分。用户观看过的所有电影都被视为相关。
...
- **矩阵分解（MF，Matrix Factorization）[8]** 是一种经典的推荐方法。我们使用与式(2)相同的训练目标来训练MF模型。
...
- **MIND（Multi-Interest Network with Dynamic Routing，动态路由多兴趣网络）[13]** 使用胶囊网络中的路由机制将用户行为分组为多个聚类，并获得多个用户嵌入用于检索。

- **ComiRec-SA（Controllable Multi-Interest Framework，可控多兴趣框架；SA：Self-Attention，自注意力）[1]** 使用多头注意力机制为每个用户生成多个嵌入，以捕捉其多样化的兴趣。

- **MMoE[18]** 是一种广泛使用的多任务学习方法。我们在实验中将自注意力块作为独立的专家网络，并使用四个专家。

#### 3.1.3 实验结果

**表2：在ML-1M和KuaiRand上的性能对比。最后一行报告了我们提出的方法相对于其基础模型SASRec的相对提升。$R@M$ 是 $\mathrm{Recall@M}$ 的缩写。**
