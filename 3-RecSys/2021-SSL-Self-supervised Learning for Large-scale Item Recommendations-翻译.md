...
**Wikipedia [14]：** 第一个数据集聚焦于Wikipedia页面之间的链接预测问题。它由页面对(x, y) \in \chi × \chi组成，其中x表示源页面，y是从x链接的目标页面。目标是从整个网页语料库中预测可能链接到给定源页面的页面集。每个页面由一个特征向量x = (x_id, x_ngrams, x_cats)表示，其中所有特征都是类别型的。这里，x_id表示页面URL的独热编码，x_ngrams表示页面标题n-gram集合的词袋表示，x_cats表示页面所属类别的词袋表示。我们按照[23]和[39]中的相同处理方式，将数据集按(90%, 10%)的比例划分为训练集和评估集。
