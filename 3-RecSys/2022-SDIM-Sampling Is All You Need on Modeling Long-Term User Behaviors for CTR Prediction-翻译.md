...
具体来说，将目标item表示为Q $\in$ ℝ^{B$$\times$$d}，用户序列表示为S $\in$ ℝ^{B$$\times$$L$$\times$$d}，其中B是每次请求中由CTR模型评分的候选item数量，L是用户行为序列的长度，d是模型的隐藏层大小。令q_i为第i个目标item，目标注意力计算q_i与行为序列S中每个item的点积相似度，然后使用归一化后的相似度作为权重来获得用户兴趣表示：
...
其中r $\in$ ℝ^d, r_i ~ N(0, 1)。h(x, r) = $\pm$1 取决于哈希输出位于哪一侧。对于两个向量x_1和x_2，我们说x_1与x_2碰撞当且仅当它们具有相同的哈希码值：
...
其中R $\in$ ℝ^{m$$\times$$d}, h(x, R) $\in$ ℝ^m。为了降低不相似item具有相同哈希码的概率，该算法将每\tau个码的结果合并，形成一个新的哈希签名。更多细节请参考[7]。在SimHash中，x_1与x_2碰撞当且仅当它们具有相同的哈希签名值，即它们在一批\tau个码中的哈希码全部相同：
