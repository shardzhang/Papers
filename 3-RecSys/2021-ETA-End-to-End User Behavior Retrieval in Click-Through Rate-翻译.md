...
**输入：** 一个d维嵌入向量 e_k \in R^{1$\times$d}
一个固定的随机哈希矩阵 H \in R^{d$\times$m}，每列可视为一个哈希函数。
**输出：** e_k的二进制签名向量 sig_k \in R^{1$\times$m}
...
**图2：我们的ETA（端到端目标注意力）模型示意图。** $e_{k+1}$ \in R^{d$\times$1}和e_t \in R^{d$\times$1}代表行为itemk+1和目标itemt的嵌入向量。d是item嵌入的维度。$h_{k+1}$和h_t是由SimHash函数生成的itemk+1和t的指纹。一个d维嵌入$e_{k+1}$可以被哈希为一个m位整数。注意，为清晰起见，图中省略了嵌入向量$e_{k+1}$和e_t在SimHash之前的投影。其他符号可在表1中找到。
...
针对不同类型的特征，我们采用不同的嵌入技术。原始输入特征主要分为两类：类别特征和数值特征。在我们的模型中，对类别特征使用独热编码。对于数值特征，我们首先将特征划分为不同的数值分桶。然后应用独热编码来标识不同的分桶，这与[16]采用的方式类似。注意，由于存在数十亿的itemID，独热编码向量可能极度稀疏。因此，我们将所有独热嵌入向量映射为低维隐藏向量，以减少参数量。我们用e_i \in R^{d$\times$1}表示itemi的嵌入向量。然后将所有用户行为item的嵌入向量打包成一个矩阵E_s \in R^{L$\times$d}，如公式3所示。L是用户行为序列的长度，d是嵌入大小。
...
其中E_t \in R^{1$\times$d}和E_s \in R^{L$\times$d}分别是目标item和行为序列的输入嵌入矩阵。L是序列长度，d是每个行为item隐藏向量的嵌入大小。为清晰起见，我们只选择一个样本而非一批样本。矩阵Q、K、V分别代表查询、键和值。d_k、d_q、d_v分别是K、Q、V中每行向量的嵌入大小。sqrt(d_k)用于避免内积值过大。softmax函数用于将内积值转换为值向量V的加权权重。W_i^Q \in R^{d$\times$d_k}，W_i^K \in R^{d$\times$d_k}，W_i^V \in R^{d$\times$d_v}。W^O \in R^{h*d_v $\times$ d}是投影矩阵。h是头数。
...
其中LTI和TA分别是长期兴趣提取单元和多头目标注意力的缩写。E_s \in R^{|H_lu|$\times$d}是长期用户行为序列H_lu的嵌入矩阵。E'_s \in R^{k$\times$d}由从E_s中选择的与目标itemE_t \in R^{1$\times$d}汉明距离最大的前k行组成。
...
对于不同数据集上的每个模型，我们使用验证集调整超参数以获得最佳性能。学习率从1$\times$10^{-4}到1$\times$10^{-2}之间搜索。L2正则化项从1$\times$10^{-4}到1之间搜索。所有模型使用Adam优化器。淘宝数据集和工业数据集的批大小分别为256和1024。
