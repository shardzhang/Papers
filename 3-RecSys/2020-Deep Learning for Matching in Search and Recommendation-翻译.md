...
> 徐军, 何向南, 李航 | 华为诺亚方舟实验室, 中国科学技术大学, 字节跳动
...
学习 和 方法 的 匹配函数学习.
基本 问题, 以及  最先进的 解决方案 的 查询-文档匹配 在 搜索 和 用户-item
匹配 在 推荐, 是 描述.  综述
...
和 contexts) 和 历史 interactions 在 item, 和 则 使
推荐 在 item   用户.  用户 特征 和 item
特征 是 索引 和 stored 在  系统 在 advance.  item 是
...
之下  uniﬁed 匹配视角 在 图 1.1, 我们 使用  术语 信息对象  表示  文档/item  检索/recommend,
...
输入 (查询, 用户, 文档, 和 item) 作为 分布式 表示, 组合中 神经网络 组件  表示  匹配
...
在 推荐, 然而, 用户 和 item 是 通常 表示了
通过 diﬀerent 类型 的 特征, 用于 例子,  特征 的 用户 可以 是
 用户 ID, age, income 水平, 和 最近 behaviors, 同时  特征
用于 item 可以 是  item ID, 类别, price, 和 brand 命名. 由于
特征 的 用户 和 item 是 从  spaces 的 diﬀerent 语义,
 naive 方法 基于 在  匹配 的 superﬁcial 特征 进行
不 工作 用于 推荐. 更多 challengingly,  item 可以 是
...
可以 如 (consume)  item 该 是 liked (消费) 通过  相似
用户, 用于 该  相似度 是 判断 从  历史 interactions (Sarwar 等人, 2001). 然而, 直接地 评估中  相似度
之间 用户 (item) suﬀers 从  稀疏性 问题, 由于  用户 仅
消费  少数 item 在  整体 item空间.  典型 假设
 解决  稀疏性 问题 是 该  用户-item交互 矩阵
是 低-排序, 该 因此 可以 是 估计 从 低-维 用户
(和 item) 潜在 特征 矩阵. 则  用户 (item) 相似度 可以 是
...
Please 注意 该 尽管 查询-文档匹配 和 用户-item
...
协同 过滤ing  信息-rich 用户-item匹配 作为 在
...
(2018b) 具有 解释了 神经 模型 用于 临时检索, 查询理解, 问答, 赞助商搜索, 和 相似 item
...
其 关系 可以 是 描述 用  集合 的 特征 \Phi(x, y).
...
函数 假设 该 r \in {0, 1} 其中 1 表明 相关 和 0
否则. 它 进一步 假设 该 f (x, y) \in [0, 1] 是  预测了
...
用户) 和 y+ 和 y− 目标 对象 (e.g., 文档 或 item). 进一步
...
P = {(x, y+, y−)|(x, y+, r+) \in D ∧ (x, y−, r−) \in D ∧ r+ (cid:31) r−},
...
其中 x 和 y 可以 是 查询 和 文档 在 搜索 和 用户 和 item
...
1, · · · , N ) 表示  查询,  文档, 和  查询-文档匹配 程度 (相关性), 分别地. 每个 元组 (q, d, r) \in D 是 生成了
...
item ij 是 生成了 根据 概率 分布 P (ij), 和
rating rij 是 生成了 根据 conditional 概率 分布
P (rij|ui, ij). 该 corresponds   fact: 用户 和 item 是 呈现了
在  推荐 系统, 和  兴趣 的  用户 在  item 是
determined 通过  已知 兴趣 的 用户 在 item 在  系统.
...
其中 ˆrij 表示  估计 ﬃnity 分数 之间 用户 ui 和 item
ij. 在 该 方式, 给定  用户,  子集 的 item 用  highest 分数
...
二 diﬀerent spaces (查询 和 文档, 以及 用户 和 item).
...
Let 我们 也 假设 该  映射 函数 是 deﬁned 作为 \phi(q) = Lqq
和 \phi0(d) = Ldd, 其中 q 和 d 是 特征 vectors 表示中 查询
...
d 是  j-th
row vectors 的 Lq 和 Ld, 和 \thetaq, \thetad, \tauq, 和 \taud 是 thresholds. | · | 和
...
下一个, 我们 brieﬂy 介绍 代表性 方法 用于 推荐 该 执行 匹配 在  潜在空间, 包括中 偏置矩阵分解 (BMF) (Koren 等人, 2009b), 因子化item相似度模型 (FISM) (Kabbur 等人, 2013), 和 分解机 (FM) (Rendle, 2010).
...
item 偏置 在 rating 分数, 和 pu 和 qi 是 潜在 vectors 表示中
 用户 和  item. 该 可以 是 interpreted 作为 仅 使用中  IDs 的
用户 和 item 作为 特征 的 它们, 和 projecting  IDs 到
潜在空间 用 二 线性 函数. Let u 是  一-hot ID 向量
的 用户 u 和 i 是  一-hot ID 向量 的 item i, 和 P 是  用户
投影 矩阵 和 Q 是  item 投影 矩阵. 则 我们 可以
express  模型 之下  映射 框架 的 方程 (2.4):

f (u, i) = h\phi(u), \phi0(i)i = h[b0, bu, 1, P · u], [1, 1, bi, Q · i]i,
...
和 \lambda 是  L2 正则化 coeﬃcient. 作为 它 是  非凸优化 问题, alternating 至少 squares (他 等人, 2016b) 或 随机
梯度 decent (Koren 等人, 2009b) 是 典型地 采用了, 该
cannot guarantee  发现  全局 optimum.

2.4.2 因子化item相似度模型

因子化item相似度模型 (FISM) (Kabbur 等人, 2013) adopts
假设 的 item-基于 协同 过滤ing, i.e., 用户 将 prefer
item 该 是 相似  什么 它们 具有 chosen 因此 far.  进行 因此, FISM

2.4. 推荐中的潜在空间模型

27

使用  item 该  用户 具有 chosen  表示  用户 和 projects
 结合 item 到  潜在空间.  模型 形式化 的
FISM 是:
...
其中 D+
u 表示  item 该 用户 u 具有 chosen, du 表示
数量 的 这样的 item, 和 d−\alpha
表示 归一化 跨越 用户.
qi 是  潜在向量 的 目标 item i, 和 pj 是  潜在向量 的
历史 item j chosen 通过 用户 u. FISM treats pT
j qj 作为  相似度
之间 item i 和 j, 和 aggregates  相似之处 的  目标 item
i 和  历史 item 的 用户 u.
...
度量, 该 测量 如何 许多 pairs 的 item 是 correctly ranked
通过  模型.
...
之间 用户 和 item, FM 也 incorporates 辅助信息 的 用户
和 item, 这样的 作为 用户 proﬁles (e.g., age, gender, etc.), item attributes
...
用于 例子, 当 x 仅 retains  用户 ID 和 目标 item ID, FM
成为  BMF 模型; 和 当 x 仅 keeps  IDs 的 用户’s
历史上 chosen item 和 目标 item ID, FM 成为  FISM
...
匹配 可以 是 进行了 在  原始 交互 空间 用 predeﬁned heuristics, 如 item-基于 CF (Sarwar 等人, 2001) 和 uniﬁed
用户-基于 和 item-基于 CF (Wang 等人, 2006). 用户-item interactions 可以 是 组织 作为  bipartite 图, 在 该 随机 walk 是
执行了  estimate  相关性 之间 任何 二 nodes ( 用户
和  item, 二 用户, 或 二 item) (他 等人, 2017b; Eksombatchai
等人, 2018). 一 可以 也 模型  生成 过程 的 用户-item
...
(i.e., 查询, 文档, 用户, 和 item) 和 学习 的 非线性
...
1, 2, · · · , N . 在 每个 位置 1 \leq t \leq N ,  bottom 解码器 组件
...
close   输入 x 作为 可能, i.e., g(f (x)) \approx x. 假设 该 我们 是
...
!" $\times
$$
%"$\times$|$|'-dim|(|-dimone-hotvectorofthecenterword)*+,)*+,-.)*-,/*)*|(|-dimone-hotvectorofthecontextwords(cid:1)%"$\times$|$|%"$\times$|$|01*+,|1*2*+,2*+,-.2*-,01*+,-.|1*01*-,|1*(cid:1)44
...
形式化地, 给定 查询 q 在  查询空间 q \in Q 和 文档
d 在  文档空间 d \in D, 函数 \phiq : Q 7\rightarrow H 和 \phid :
D 7\rightarrow H 表示 映射 从  查询空间 和 映射 从
 文档空间   新 空间 H, 分别地.  匹配
函数 之间 q 和 d 是 deﬁned 作为

f (q, d) = F (\phiq(q), \phid(d)),
...
查询 和 文档 (i.e., \phiq = \phid). 它们 可以 是 广义  具有
...
其中 W1, b1, W2, 和 b2 是 参数, 和 \sigma(·) 是  sigmoid
函数.
...
i(t) =\sigma(W3l(t) + Wrec3y(t − 1) + Wp3c(t − 1) + b3),
f (t) =\sigma(W2l(t) + Wrec2y(t − 1) + Wp2c(t − 1) + b2),
c(t) =f (t) (cid:12) c(t − 1) + i(t) (cid:12) u(t),

o(t) =\sigma(W1l(t) + Wrec1y(t − 1) + Wp1c(t) + b1),
...
\phingram(wi, wi+1, · · · , wi+n−1),

其中 w1, w2, · · · , w|d| 表示  词 序列 在 d 和 \phingram 表示  高-维 和 稀疏 表示 的  n-gram
wi, wi+1, · · · , wi+n−1. 该 是, \phi 首先 converts  n-gram 的 词 到
...
\phingram(qi, qi+1, · · · , qi+n−1),

其中 qi, qi+1, · · · , q|q| 表示  词 序列 在 q 和 \phingram 表示  高-维 和 稀疏 表示 的  n-gram
...
d1 和 d2,  偏好 标签 z \in {−1, 1} 表明 该 文档
...
在 该 item 在 diﬀerent 模态 是 表示了 和 比较了,
...
用于 item 从 diﬀerent 模态 在  共同空间. 它 具有
目标 的 confusing  模态 classiﬁer 作为  adversary.
模态 classiﬁer 是 responsible 用于 distinguishing item 从 其
...
输入.  深度神经网络, 表示为 作为 fT (·; \thetaT ), 是 使用了  进行
文本 特征 投影.  图像 分支 的 ACMR 取 CNN 图像
特征 作为 输入.  深度神经网络, 表示为 作为 fV (·; \thetaV ), 是 使用了
 进行 图像 特征 投影. \thetaV 和 \thetaT 是  参数 在
 二 网络.
...
作为 follows.
 ...
 模态 classiﬁer D 是  馈 前向 神经网络 用 参数 \thetaD 该 预测  概率 的 模态 给定  实例
...
item 从 其 模态.  特征 projector manages  confuse  模态
...
(mi · (log D(fV (vi); \thetaD) + log(1 − D(fT (ti); \thetaD)))) ,
...
(ˆ\thetaV , ˆ\thetaT , ˆ\thetaimd) = arg min

\thetaV ,\thetaT ,\thetaimd

(Lemb(\thetaV , \thetaT , \thetaimd) − Ladv(ˆ\thetaD)),

ˆ\thetaD = arg max
\thetaD

(Lemb(ˆ\thetaV , ˆ\thetaT , ˆ\thetaimd) − Ladv(\thetaD)).
...
[s0, s1]T = f (q, d) = W2\sigma (W1z + b1) + b2,
...
{(qn, dn, rn)}N
n=1 其中 rn \in {0, 1} 是  真实值 标签.  crossentropy 损失 是 计算了:
...
和 \sigmak 是  均值 和 方差 的  RBF 核, 分别地.
...
2004) 和 语义 相关 匹配 (Rasiwasia 等人, 2010) 是 传统 模型. 两者 的  模型 aim  学习 线性 transformations
 item 二 对象 在 diﬀerent 模态 到  共同空间 这样的
...
web. MatchZoo9 是  open-来源 item dedicated  研究 在 深度
...
采用了  创建 表示 的 用户 和 item  使  比较 之间 它们 和 生成  ﬁnal 匹配分数. 在  秒
类别, 神经 网络 是 利用了  进行 interactions 之间
用户 和 item (和 possibly contexts)  生成 匹配 signals 和
...
给定 用户 u 在  用户空间 u \in U 和 item i 在  item空间 i \in I,
函数 \phiu : U 7\rightarrow H 和 \phii : I 7\rightarrow H stand 用于 mappings 从  用户
空间 U 和 从  item空间 I   新 空间 H, 分别地.
...
corresponds   item. 我们 下一个 综述 三 类型 的 方法 该
...
学习 item 表示. 在 每个 tower,  MLP 是 采用了
...
具有 interacted 用 item i 在之前. Let yu∗ \in RN 表示  u-th row
的 Y, i.e.,  多-hot 历史 向量 的 用户 u, 和 y∗i \in RM 表示

pu…qiInteractionmatrixYy*iyu*多-layernonlinearprojection…LatentrepresentationmatchingscoreLayer1层1LayerNLayerN102

推荐中的深度匹配模型

 i-th column 的 Y, i.e.,  多-hot 历史 向量 的 item i. 则,
...
作为  spaces 的 用户 和 item 是 diﬀerent, DeepMF 使用 二 MLPs
用 diﬀerent 参数  表示 用户 和 item. 注意 该 due
...
是 几乎 able  recover  输入 数据. 在 item-基于 AutoRec (Sedhain
等人, 2015),  输入 是  用户 历史 向量 yu∗ \in RN , 和
重建 的  输入 是:
...
\theta = {V, W, b1, b2}, AutoRec minimizes  总计 损失 超过 所有 输入
(用户) 用 L2 正则化:
...
lookup  获得  item 表示.  是 更多 speciﬁc, 我们 可以
reformulate 方程 (5.3)  get  元素-wise 匹配函数:

f (u, i) = ˆyu∗,i = \sigma2(wi∗
|{z}
qi
...
嵌入 的 item i, 和 用户 表示 pu 是 等价
输出 的  一-层 MLP 用 yu∗ 作为 输入.  匹配分数 是
本质上  内积 的 用户 表示 pu 和 item ID
...
一 观察 在  学习 的 用户 表示 是 该 历史 item 可以 不 equally contribute   建模 的  用户’s
偏好. 用于 例子,  用户 可以 选择  trendy item 基于 在 其
...
无 guarantee 用于 该.  solve  问题,  神经 Attentive item
相似度 (NAIS) 模型 (他 等人, 2018) 采用  神经 注意力
网络  显式地 学习  权重 的 每个 历史 item. 图 5.2
...
在 每个 interacted item 的  用户. Let Yu 是  集合 的 interacted item
的 用户 u, 和 每个 item i 是 associated 用 二 ID 嵌入 vectors
pi 和 qi  表示 其 角色 作为  目标 item 和  历史 item,
...
item j 在 估计 的  用户 u’s 匹配分数 在  目标 item
...
attentionoutputattentioninputinnerproduct010101…000100…q2q4q6p3s32s34s36attentionnetworka323436\summatchingscore5.1. 基于表示学习的匹配
...
item i 当 estimating  权重 的  历史 item j. 该 purposeful
设计 是  解决  局限性 的 静态 用户 表示 当
interacting 用 diﬀerent item. 用于 例子, 当  用户 考虑
是否  purchase  clothing item,  历史 behaviors 在
...
的 动态 (目标 item-aware) 用户 表示. 它 是 显示  是
有用  distill 有用 signals 从 用户 行为 历史 在 大-规模
e-commerce CTR 预测.

5.1.2 表示学习 从 序列 Interactions

用户-item interactions 是 自然地 associated 用 timestamps, 该
记录 当  交互 happens. 如果  顺序 的 用户-item interactions
是 考虑了,  交互 历史 成为  序列 的 item IDs.
建模 这样的  序列 可以 是 有用 用于 预测 的 用户 行为 在
 未来, 用于 例子, purchase 转移 patterns 从 一 item (e.g.,
phone)  另一个 item (e.g., phone 案例) exist 和 最近 purchases
...
作为 一 的  pioneering 工作 在 RNN 用于 基于会话的推荐, Hidasi 等人 (2016) 提出  GRU-基于 RNN 用于 summarizing
 序列 interactions (e.g.,  序列 的  clicked item) 在
 会话 和 使 推荐, 称为 GRU4Rec.  输入
  GRU4Rec 模型 是  序列 的 r clicked item 在  会话

106

推荐中的深度匹配模型

x = (x1, · · · , xr−1, xr), 其中 每个 item 是 表示了 作为  一-hot
N -维 向量 和 N 是  数量 的 item.  输出 是
 下一个 事件 (clicked item) 在  会话. 更多 speciﬁcally, 在 每个
位置 i 在  序列 x,  输入 是  陈述 的  会话, 该
可以 是  一-hot 表示 的  当前 item, 或  weighted sum
的 表示 的 item 因此 far, 作为 显示 在 图 5.3. 作为  core 的
 网络,  多-层 GRU 是 使用了  receive  embeddings 的
输入 表示, 和  输出 的 每个 GRU 层 是  输入
  下一个 层. 最后, feedforward 层 是 added 之间
最后 GRU 层 和  输出 层.  输出 是  N -维
向量, 每个 表示中  概率 的  相应 item 是
clicked 在  下一个 事件 的  会话.

期间 训练, 成对 排序 损失 是 利用了  学习
模型 参数. 二 类型 的 损失 函数 是 使用了.  BPR 损失
比较  分数 的  正 (preferred) item 用 那些 的 若干
sampled 负 item. 因此,  BPR 损失 在  位置 是 deﬁned 作为:
...
log(\sigma(ˆrs,i − ˆrs,j)),

其中 Ns 是  数量 的 sampled 负 item, ˆrs,i (或 ˆrs,j) 是
预测了 分数 的 item i (或 j), i 是  正 item, 和 j 是
负 item.  其他 类型 的 损失 称为 TOP1 是 也 devised, 该
...
更多 流行  item 是,  更多 可能  用户 knows 关于 它, 用于
生成中  负 item.
...
图 5.3: 模型 架构 的 GRURec. 它 processes 一 item 的  item
序列 在 一旦.
...
x = (x1, x2, · · · , xt) consisting 的 t clicked item,  全局 编码器
...
t
 ...
 uniﬁed 序列 表示 在 位置 t, 以及 嵌入 的 候选 item, 是 fed 到  解码器.  相似度 之间
 序列 表示 在 位置 t 和 嵌入 的 候选
item i 是 计算了 作为  双线性 函数:
...
在  m item 分数  生成  分布 (的 点击) 超过 所有
候选 item, 其中 m 是  数量 的 candidates.

 学习  模型 参数, 交叉-熵 损失 是 使用了. Speciﬁcally,
给定  训练 序列, 在  位置 t NARM 首先 预测
概率 分布 超过  m item qt. 从  log 我们 know
...
其中 pi
t 是  预测了 概率 和 ground-真值 概率 用于 item i, 分别地.  损失函数 可以 是 优化 用
 标准 mini-批处理 SGD.
...
(Tang 和 Wang, 2018).  基本 想法 是  treat  interacted item
...
另一个 问题  是 注意到 是 该 当 recommending  下一个 item
的 一 序列, 仅  之前 item 是 已知 (see 图 5.7). 该
...
在 最后,  输出 的  最后 自身-注意力 块 是 使用了 用于 预测. 给定  历史 item 序列 {v1, v2, . . . , vt},  下一个 item
需求 是 预测了 基于 在 F(L)
, 其中 L 是  数量 的 块.
预测了 分数 的  目标 item i 是:
...
Chua, 2017) 用  用户 (item) ID 嵌入. 最后,  pooled
用户 向量 和 item 向量 是 结合 到  MLP  获得
预测 分数:
...
和 item 属性, Vu 和 Vi 表示  数量 的 attributes 用于
用户 u 和 item i, 分别地.  bi-交互 池化 操作
...
考虑 所有 成对 interactions 之中 用户 ID 嵌入 和 属性 embeddings.  结合 用户 表示 \phiu(u) 和 item
表示 \phii(i) 是 interacted 通过 元素-wise 乘积, followed
通过  MLP 用于  ﬁnal 预测.  MLP 是  learnable 匹配
函数 (更多 细节 将 是 介绍 在 节 5.2), 该 可以
也 是 replaced 用 简单 内积.  优势 的 该 架构 是 该  interactions 之间 用户 (item) attributes 和
 交叉-interactions 之间 用户 attributes 和 item attributes 是
好 捕获了.

Li 等人 (2015) 提出  正则化-基于 方法  incorporate attributes 到 推荐.  想法 是  首先 学习 表示 从 用户 特征 和 item 特征 用 二 auto-encoders,
...
U 作为 用户 表示 和 L(X, U ) 作为 损失函数;  正确 自编码器 是 构建了 从 item 特征 Y 用  隐藏层 V 作为 item
表示 和 L(Y, V ) 作为  损失函数. 则 U 和 V 是
使用了  reconstruct  用户-item 评分矩阵 R 用于 推荐 用 l(R, U, V ) 作为 损失函数.  整体 模型 是 训练了 通过
...
学习 模型  联合地 学习 item properties 和 用户 opinions 从
 综述, 称为 深度 Cooperative 神经 网络 (DeepCoNN).
作为 显示 在 图 5.10, DeepCoNN 由组成 的 二 并行 神经

InputLayerHiddenLayer$\times$UserFeatureXUserFeatureY#V\approxOutputLayerUser-ItemRatingMatrixRr'(,*'+,,'-,*,,$\times
$$
\times
$  $
\times$ 118
...
(称为 N etu), 和  其他 学习 的 item properties 从  综述
...
类似地, 给定 所有  综述 用于  item i, N eti 也 merges
综述 到  单一 文档 di
1:m 的 m 词, 创建  矩阵 的
词 vectors V i
1:m, 和 采用  一-维 CNN  总结
...
119
 ...
 ﬁnal 匹配分数 的 用户 u 和 item i 是 计算了 在  基础 的
...
其中 (cid:12) 表示 元素-wise 乘积, qu 和 pi 分别地 表示
 用户 preferences 和 item 特征, w1 是  权重 向量, bu, bi
和 µ 是  用户 偏置, item 偏置, 和 全局 偏置, 分别地.
...
综述 written 通过  用户⋯CNNreview1𝑉#,%𝐱#CNNCNNreview2reviewn𝑉#,’𝑉#,(𝑂#%𝑂#’𝑂#(𝑖#%𝑖#’𝑖#(用户'综述'注意力⋯𝑎#%𝑎#’𝑎#(\sum𝐪#(useridembedding)+综述 written 用于  item⋯CNNreview1𝑉#,%𝐱.CNNCNNreview2reviewm𝑉#,’𝑉#,/𝑂#%𝑂#’𝑂#/𝑢.%𝑢.’𝑢./item'综述'注意力⋯𝑎.%𝑎.’𝑎./\sum𝐪.(itemidembedding)+元素-wiseproductpredictionlayer𝑦#.5.1. 基于表示学习的匹配

121

矩阵 E, 该 是, \thetai = Egi. 它 则 concatenates \thetai 用  item ID
嵌入 qi  形式  ﬁnal item 表示. 最后, 它 interacts
 item 表示 用  用户 表示 用 内积
 获得  预测 分数, 该 是, ˆyui = \phiu(u)T [qi, Egi]. 注意 该
...
然而, 作为  数量 的 用户-item interactions 是 典型地 许多
...
该 share  相同 想法 — reﬁning  extracted 图像 表示 在  用户-item交互 图 用  图 卷积
网络.  extracted 图像 表示 是 treated 作为  初始 特征 的 item nodes, 该 是 propagated 在  交互
图 用  图 卷积 操作. 由于  交互 图
结构 包含 用户 偏好 在 item 特别  协同
...
基于 在 用户-item interactions,  图像 表示 是 适应
...
5.1.4 表示学习 从 图 数据
 ...
 以上-提及了 表示学习 方法 具有  缺点 — 学习 从  信息 的  用户 ( item) 分别地,
同时  relations 之中 用户 和 item 是 ignored.  用户-item
交互 图 提供 rich 信息 在 用户 和 item relations,
和  item 知识图谱 提供 rich 信息 在 item relations, 和 因此 学习 表示 从 这样的 graphs 可以 overcome
 缺点 和 具有  potential  改进  准确率 的 推荐. 若干 最近 工作 try  利用 该 信息
和 开发了 图 表示学习 基于 推荐 系统 (Wang 等人, 2019b; Wang 等人, 2019; Ying 等人, 2018; Wang
等人, 2019c). 用户-item interactions 是 组织 作为  bipartite 图,
social relations 之中 用户 是 呈现了 在 social 网络, 和 item
知识 (e.g. item attributes 和 relations) 是 表示了 在  形式
的 知识图谱 (aka. 异质信息网络). 这样的
 图 结构 connects 用户 和 item, opening 向上 possibilities
...
由于 用户-item interactions 可以 是 表示了 在  bipartite 图,
...
刻画 用户 和 item —  用户’s interacted item 给出 支持中
证据 在  用户’s 偏好, 同时  item’s associated 用户 可以
是 viewed 作为 特征 的  item. 此外, 高-顺序 connectivities
reﬂect 更多 复杂 patterns —  路径 u1 \leftarrow i1 \leftarrow u2 表明
behavioral 相似度 之间 用户 u1 和 u2, 作为 两者 具有 interacted
用 item i1;  longer 路径 u1 \leftarrow i1 \leftarrow u2 \leftarrow i2 建议 u1’s 偏好
在 i2, 由于 他的/她的 相似 用户 u2 具有 采用 i2. 图 5.12 显示
例子 的 这样的 高-顺序 connectivities, 该 reﬂect  用户-用户
和 item-item dependencies. NGCF 旨在  inject 这样的 signals 到
表示 的 用户 和 item.

图 5.12:  例子 的 高阶连通性 revealed 在 用户-item交互
...
bipartite 用户-item交互 图. 图 5.13 显示 其 框架.
...
\rho(·) 是  非线性 激活函数, 和 Nu 是  neighbor 集合 的
u; m(l)
u\leftarrowj 是  消息 是 propagated, \alphauj 是  decay 因素 用于
...
该 在  用户-item交互 图, 每个 节点 (用户 或 item) 是 仅
...
除之外 用户-item interactions, 更多 最近 工作 也 取
到 consideration  relations 之中 item 在  知识图谱.
知识图谱 (KG) 是  强大 资源 该 提供 rich 侧
信息 在 item (i.e. item attributes 和 item relations), 其中
nodes 是 entities 和 edges 表示  relations 之间 它们.
通常 KG 组织 facts 或 beliefs 在  异质 directed 图
G = {(h, r, t)|h, t \in E, r \in R}, 其中  triplet (h, r, t) 表明 该
那里 是  关系 r 从 head 实体 h  tail 实体 t. 用于 例子,
(Hugh Jackman, ActorOf, Logan) 陈述  fact 该 Hugh Jackman 是
 actor 的  movie Logan.

 使用 的  知识图谱 可以 增强  学习 的 item
表示 和 建模 的 用户-item relationships. 在 particular,
...
可以 帮助  原因 关于 unseen 用户-item interactions (i.e.  potential
推荐).

朝向  端, 知识图谱注意力网络 (KGAT)
(Wang 等人, 2019) 扩展 NGCF 通过 adaptively extracting 信息
从  neighborhood 的 高阶连通性. Diﬀerent 从 NGCF
其中  decay 因素 \alphaht 在 propagation 的 信息 在 边 (h, t)
...
实体 h, \alpha(h,r,t) 是  attentive decay 因素 derived 从  注意力 网络 g(·), 表明中 如何 许多 信息 是 propagated 和
identifying 重要性 的 neighbors 用 regard  关系 r. 之后
establishing  表示, KGAT 使用  相同 预测 模型
作为 方程 (5.16)  estimate 如何 可能  用户 将 adopt  item.
...
之间 用户 和 item. 在 particular,  模型 首先 或 deﬁne
...
推荐中的深度匹配模型
 ...
 用户 u,  目标 item i, 和  集合 的 paths P(ui) = {p1, · · · , pK}
...
则 feeds 它 到  MLP  获得  ﬁnal 分数 用于  用户-item
...
5.2 基于匹配函数学习的匹配
 ...
 匹配函数 输出  匹配分数 之间  用户
和  item, 用 用户-item交互 信息 作为 输入, 沿着
用 可能 辅助信息 包括中 用户 attributes, item attributes,
contexts, 和 others. 我们 分类  方法 到 二 类型 基于
在  输入   匹配函数 — 二-方式 匹配 (仅
用户 信息 和 item 信息 是 提供) 和 多-方式
匹配 (其他 辅助信息 是 也 提供).

5.2.1 二-方式 匹配

传统 潜在空间 模型 计算 内积 或 余弦相似度 之间 用户 和 item  获得  匹配分数. 然而,
...
是  位置  多-层 神经网络 以上 用户 嵌入 和 item
嵌入  学习 其 交互 分数:

f (u, i) = F (\phiu(u), \phii(i)),

其中 F 是  交互 神经网络  是 speciﬁed, \phiu(u) 和
\phii(i) 表示  embeddings 的 用户 u 和 item i, 分别地. 若干
实例 是 提出 之下  NCF 框架:

- 多-层 感知 (MLP).  straightforward 方式 是  stack
 MLP 以上  concatenation 的 用户 嵌入 和 item 嵌入, leveraging  non-线性 建模 能力 的 MLP  学习
交互 函数: F (\phiu(u), \phii(i)) = M LP ([\phiu(u), \phii(i)]). 虽然
...
操作, 该 是, 然而, very 重要 用于 建模 的 交互 在 CF (相应   低-排序 假设 的 用户-item
交互 矩阵). 它 是 重要, 因此,  显式地 express
multiplication 或 相似 eﬀect 在  匹配 网络.

- 广义 矩阵分解 (GMF).  泛化 MF 之下  NCF 框架,  authors 的 NCF 首先 计算 elementwise 乘积 在 用户 嵌入 和 item 嵌入, 则 输出
 预测 分数 用  完全地 connected 层: F (\phiu(u), \phii(i)) =
\sigma(wT ([\phiu(u) (cid:12) \phii(i)])). w 是  trainable 权重 向量 的  层,
...
和 item 嵌入, followed 通过  CNN  aggregate  interactions
hierarchically. 图 5.17 说明  模型.  输出 的  outer
乘积 是  2D 矩阵, 其中  (k, t)-th 条目 是 (pu \otimes qi)kt = puk · qit,
捕获中  交互 之间  k-th 维度 和  t-th
维度 (pu 和 qi 表示 用户 嵌入 和 item 嵌入).
...
000100…000010…MFUserVectorMLPUserVectorMFItemVectorMLPItemVectorGMFLayerMLPLayer1MLPLayerNNeuMFLayer…concatenationelement-wiseproductReLUReLUconcatenation!"# $"#$ trainingloglossuser%item&scoretarget132
...
的  dissatisfaction 的 triangle inequality. 作为  结果, 它 是 不 able
 捕获 ﬁner-grained 用户 preferences 和 用户-用户 和 item-item
...
用户-item 度量 在 CML 是 deﬁned 作为  欧氏距离 之间
用户 嵌入 和 item 嵌入:
...
其中 pu 是  用户 嵌入 向量, qi 是  item 嵌入 向量,
和 d(u, i) 是  distance 之间 用户 u 和 item i,  smaller,
...
和 item-item relationships 从 用户-item interactions.  直觉
是 该  item 该  用户 likes 是 close   用户 比  其他
item 该  用户 进行 不 如. 因此,  间隔-基于 成对 损失 是
deﬁned 作为:
...
wui[\delta + d(u, i)2 − d(u, j)2]+,

(5.22)

其中 i 表示  item 该 u likes, j 表示  item 该 u 进行 不
如, \delta > 0 是  predeﬁned 间隔 大小, [z]+ = max(z, 0) 表示
hinge 损失函数, 和 wui 是  权重 的 训练实例 该
是 predeﬁned.  authors 提出 若干 additional constraints
改进  质量 的  学会了 metrics, 包括中  bound 的 用户
嵌入 和 item 嵌入 之内  单元 sphere (i.e., ||p∗|| \leq 1
和 ||q∗|| \leq 1), 和  regularizer  de-correlate  dimensions 的
学会了 度量. 我们 指代  读者   原始 论文 用于 更多
细节 (Hsieh 等人, 2017).

- 翻译-基于 推荐 (TransRec) (他 等人, 2017).
TransRec 可以 是 seen 作为  扩展 的 CML 用于 下一item推荐, 该 accounts 用于 用户 序列 行为 通过 建模
第三-顺序 交互 之中  用户,  之前 visited item, 和
 下一个 item  visit (Rendle 等人, 2010).  想法 是 该,  用户
是 表示了 作为  “翻译 向量”, 该 translates  之前
item   下一个 item, i.e., qj + pu \approx qi.  distance 度量  realize
 翻译 是:
...
则 estimate  似然 该  用户 使  转移 从 item
j  item i 作为:

prob(i|u, j) = \betai − d(qj + pu, qi),
...
它 和  下一个 item. 最近, Wu 等人 (2019) 扩展 TransRec 通过
建模 多个 之前 item 和 高-顺序 interactions 用 它们.
- 潜在 Relational 度量 学习 (LRML) (Tay 等人, 2018).
LRML 进展 TransRec 通过 进一步 学习  关系 之间
 用户 和  下一个 item.  优势 是 该  度量 是 更多
...
uniform r 用于 所有 用户-item pairs, LRML parameterizes 它 作为  attentive sum 超过 external 记忆 vectors. 图 5.18 显示  模型
...
网络 该 取  交互 之间 用户 嵌入 和 item
嵌入 作为 输入. 在 该 方式,  关系 向量 是 用户-item交互 aware, 该 增加  geometrical ﬂexibility 的  度量.
...
如 watched item, 它 执行 average 池化  获得  序列
...
原始 输入 特征 和 \phi(x) 是  构建了 交叉 特征. 则
 模型 用于 预测中  点击率 是:
...
2017) ensembles FM 和 MLP 通过: ˆyDeepF M = \sigma(ˆyF M + ˆyM LP ), 其中
...
在  用户-item 图 用 特征 的 每个 模态 (visual, textual,
和 acoustic), 和 fuses  输出 的 每个 模态  get  ﬁnal
表示 的  micro-视频. 用 regard  表示学习
在 图 数据, 除之外  用户-item 图 和 知识图谱 作为 我们
...
item, 注意力 网络 是 designed  学习 用户’s speciﬁc preferences
 diﬀerent aspects 的 item 通过 leveraging 综述 和 images (Cheng
...
到 account relations 的 多个 item (e.g., 相同 类别, 共享
属性, etc.) 用于 item-基于 CF. Gao 等人 (2019) 开发  privacypreserving 方法 用于 交叉-领域 推荐 通过 transferring
用户 embeddings 而 比 raw 行为 数据. Pan 等人 (2019) tailor
 学习 的 嵌入 参数 用于 item cold-开始 recommenda-
...
和 cons. 除之外 interacting 用 用户 用 item recommendations,
最近 工作 (Zhang 等人, 2020) 考虑 asking 属性 偏好,
该 可以 发现  大多数 相关 item 更多 eﬀectively. 神经 网络
...
该 由组成 的 用户-item交互 数据. Yelp4, Ciao5, 和 Epinions6 (Richardson 等人, 2003) 是 datasets 该 此外 包括
...
综述 和 comments 在 item, 它们 是 广泛 利用了 在 综述-基于
...
Simiarly, OpenRec17 是  open-来源 item 该 包含 若干
推荐 方法.
...
发现. 在 推荐,  用户 和  item belong  diﬀerent
类型 的 entities 和 是 表示了 通过 diﬀerent superﬁcial 特征,
使 它 diﬃcult  进行 匹配 之间  特征 和 因此
oﬀer satisfactory recommendations 在 item  用户.  桥接
...
item 基于 在 什么  用户 具有 chosen, 或 通过 asking 用户
什么 种类 的 item attributes 它们 prefer (Lei 等人, 2020). 因此, 如何  结构  用户-系统 interactions 和 进行
查询-文档 (或 用户-item) 匹配 在  交互 和
...
3+ 十亿 item  200+ 百万 用户 在 真实-时间”. 在: 会议论文集 的  2018 世界 广泛 Web 会议 在 世界 广泛
...
神经 Attentive item 相似度 模型 用于 推荐”. IEEE
...
他, X., M.-Y. Kan, P. Xie, 和 X. Chen. 2014. “评论-基于 Multiview 聚类 的 Web 2.0 item”. 在: 会议论文集 的  23rd 国际 会议 在 世界 广泛 Web. WWW ’14. Seoul, Korea:
...
Kabbur, S., X. Ning, 和 G. Karypis. 2013. “FISM: Factored item
...
Qiu, R., J. Li, Z. Huang, 和 H. Yin. 2019. “Rethinking  item 顺序
在 基于会话的推荐 用 图 神经 网络”.
...
Sarwar, B., G. Karypis, J. Konstan, 和 J. Riedl. 2001. “item-基于 协同过滤 推荐 算法”. 在: 会议论文集
...
Wang, X., X. 他, L. Nie, 和 T.-S. Chua. 2017b. “item Silk Road:
Recommending item 从 信息 Domains  Social 用户”.
...
Wu, B., X. 他, Z. Sun, L. Chen, 和 Y. Ye. 2019. “ATM:  Attentive 翻译 模型 用于 下一item推荐”. IEEE
...
Xin, X., X. 他, Y. Zhang, Y. Zhang, 和 J. M. Jose. 2019b. “Relational 协同过滤: 建模 多个 item Relations 用于
...
“ 简单 卷积 生成 网络 用于 下一个 item 推荐”. 在: 会议论文集 的  Twelfth ACM 国际
...
Zamani, H. 和 W. B. Croft. 2020. “学习  联合 搜索与推荐 模型 从 用户-item Interactions”. 在: 会议论文集 的
