...
s̄ = s($\zeta$ − \gamma) + \gamma
z = min(1, max(s̄, 0))
$$

其中u是一个均匀随机变量，log(\alpha)是可学习的分布参数，\beta, \gamma, $\zeta$均为超参数。关于硬具体分布的更多细节可参见Louizos, Welling和Kingma（2017）。
...
$$
ẑ = min(1, max(0, sigmoid(log(\alpha))($\zeta$ − \gamma) + \gamma))
$$

当sigmoid(log(\alphaᵢⱼ))($\zeta$ − \gamma) + \gamma < 0时，ẑᵢⱼ = 0，得到的模型将具有稀疏连接。为了在SNR-Aver模型中减少服务模型参数大小，我们需要从模型中移除至少一个完整的子网络，这意味着我们需要ẑᵢⱼ = 0（对所有i成立）才能消除第j个子网络。然而对于SNR-Trans，任何ẑᵢⱼ = 0都会消除对应的Wᵢⱼ。因此SNR-Trans比SNR-Aver更容易减少模型参数。
