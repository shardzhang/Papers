# GenCTR: Generative Click-through Rate Prediction with Applications to Search Advertising

> Lingwei Kong, Lu Wang, Changping Peng, Zhangang Lin, Ching Law, Jingping Shao | JD.com
...
下一个item类别的下一item预测；2）在判别式点击率预测框架内微调
训练良好的生成式模型。
...
收入模式下，广告商为其赞助item的每次点击付费。点击率
预测模型分析历史广告展示和点击日志，以估计用户
点击给定item的概率。这些估计随后与广告出价一起用于确定
...
网络（DIN）是这一领域的开创性工作，
引入了注意力机制，在预测不同item的点击率时对用户的历史行为进行差异化加权。
该模型能够捕捉过去交互相对于当前广告或item的不同相关性。随后，研究人员探索了两阶段方法，
...
序列推荐系统旨在通过考虑过去交互的顺序来预测用户可能交互的下一个item。
这些模型的演变始于卷积神经网络（CNN）和循环神经网络（RNN）
在捕捉用户-item交互序列模式中的应用。基于CNN的模型，如Caser（Tang & Wang，
...
我们用X表示输入空间，包含与用户、item以及
相关上下文相关的所有特征。输出空间Y = {0, 1}，表示是否发生点击。令D表示
X $\times$ Y上的潜在分布。点击率（CTR）预测旨在开发一个
模型ŷ：X \rightarrow (0, 1)。该模型估计对于任何给定输入x \in X发生点击的概率（即点击率），
其中ŷ(x)本质上预测P$r_{y~D_{Y|x}$}{y = 1 | x}。
每个用户都可以拥有一个独特的用户行为序列，表示他们与item交互的时间顺序记录
（例如，点击、购买和将item加入购物车）。该序列可以
进一步通过附加到每个item的辅助信息进行丰富，提供更深层的上下文。令A为
item空间，C为与这些item对应的辅助信息空间。那么，
用户行为的序列空间表示为(A $\times$ C)∗。通过将该序列作为
...
模型旨在通过生成一个捕捉其潜在特征的嵌入来预测序列中的下一个item。
该嵌入是item的数值表示，通常
用h \in Rd表示，其中d表示嵌入维度。

条件生成式模型
我们不单独依赖item的历史序列，而是
提出通过增强生成式模型来考虑与下一个item相关的辅助信息
以改善行为序列的利用。在预测下一个元素时，该辅助信息
与前面的item一起作为额外输入。本质上，我们的模型
变得条件依赖于这个传入的辅助信息。在数学上，我们可以将
生成式模型表示为函数g：(A $\times$ C)∗ $\times$ C \rightarrow Rd。

3

条件自注意力解码器 我们的条件生成式模型实现为
自注意力解码器的一个变体。在传统的自注意力解码器中，查询向量源自
最近的item，而键和值向量源自它之前的item。
相比之下，我们的条件自注意力解码器采用了一种不同的方法——其查询向量
源自与下一个item相关的辅助信息，而键和值向量则来自
当前item的前面item（包括最近的item）。我们将该方法
...
对应于条件自注意力机制，负实例从与下一个item具有相同辅助信息
的item中采样。

损失函数 考虑一个用户行为序列。对于每个item，解码器掩码该item
（保留用于查询向量的辅助信息）及其后续item。然后它预测
当前item的嵌入。真实标签就是item本身，而负标签
...
其中x \in X是骨干点击率模型的原始特征，s \in (A $\times$ C)∗是用户
行为序列，c是目标item的辅助信息，集成的点击率
...
ℓ(y, ŷ(x, g(s, c; \theta)); \theta),

(1)

其中ℓ(·, ·)是交叉熵损失，\theta包含骨干点击率预测
模型和生成式模型的所有参数。
...
（均值$\pm$标准差）。$\uparrow$表示数值越大越好；$\downarrow$表示数值越小越好。
...
测试集来自随后一天的日志。
数据集包含详细的item信息、用户画像以及用户最近200次点击item及
相应辅助信息。

除了主要的训练集和测试集，我们还整理了一个预训练集和一个
类别item采样表。预训练集
通过对训练集中的用户条目进行去重生成，突出用户画像和之前的浏览活动。类别item采
样表则记录了训练集中出现的所有产品类别与其对应
item的映射。我们后续的离线实验是在此
...
这证实了通过我们的预训练过程细化的item表示对点击率估计具有显著的正向效果。
