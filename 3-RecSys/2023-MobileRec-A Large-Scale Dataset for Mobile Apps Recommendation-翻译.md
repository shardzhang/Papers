...
> M.H. Maqbool, Umar Farooq, Adib Mosharrof, A.B. Siddique, Hassan Foroosh | University of Central Florida; Independent Researcher; University of Kentucky

本文介绍了 MobileRec：面向移动应用推荐的大规模数据集。核心内容：

- 数据集规模：基于用户在 Google Play 商店中的活动构建，包含 1930 万条用户交互（应用评分与评论），涵盖 48 个类别中的超过 1 万个独立应用，共 70 万唯一用户，每个用户与不少于 5 个应用交互
- 数据特征：提供用户对已安装应用的评分与情感信息，以及应用名称、类别、描述、总体评分等丰富元数据，并保留交互时间戳与匿名化用户唯一标识
- 实验验证：对多种最先进推荐方法进行对比研究，证明 MobileRec 可作为应用推荐的优秀测试平台，并建立定量基准结果
- 数据获取：数据集公开获取地址为 https://huggingface.co/datasets/recmeapp/mobilerec

关键发现：

- 用户行为高度动态：用户在内容分级、价格层级、评分区间和 top-5 类别之间的迁移模式波动剧烈，给推荐系统中用户行为建模（尤其是冷启动场景）带来挑战
- 动态交互压缩性能增益：相比 Amazon-Beauty 等数据集，各基线在 MobileRec 上的相对提升普遍更小，例如 ELECRec 相对 GRU4Rec 在 Hit@5 上的提升由 329.87% 降至 9.30%
- 结构优势：与现有移动应用数据集不同，MobileRec 记录每个用户的多次交互（不少于 5 次）、每个应用至少有 15 条交互，并保留用户唯一标识与时间戳，是移动推荐系统的理想测试平台
...
Sequential Recommendation, GooglePlay Dataset, App Recommendation Dataset.
...
推荐系统在广泛领域展现了显著的有效性，例如新闻[46]、电影[10]、产品[42, 48]、时尚[21]、健身[34]、玩具、美妆、CD[18, 31, 33]等等[37]。包含用户-item交互的特定领域数据集的可用性[1, 26, 30]在推荐系统的发展和改进中发挥了关键作用。在早期研究中，item和用户通常用其唯一标识符表示，它们各自的交互（如评分分数）被用来学习有助于推荐任务的有用关系。这些关系通常通过协同过滤技术或类似方法[26]来学习。最近，将序列化用户交互纳入推荐系统的开发已显著提升了它们的性能[23]。通过考虑用户与item交互的序列模式[33]（例如，他们查看或购买item的顺序），这些方法可以更好地捕捉用户偏好并提供更准确的推荐[23]。此外，诸如循环神经网络（RNN，Recurrent Neural Network）[11, 19]和基于注意力的模型[43]等有效处理序列数据的新技术，在推荐任务中表现出了卓越的性能[42, 48]。

有几个值得注意的移动应用数据集，例如 Top 20 apps 数据集[3]、RRGen[15]、AARSynth[13]、Srisopha 等人[38]和 PPrior[14]等。然而，Top 20 Apps 和 RRGen 只包含有限数量的评论，通常在数十万条左右。此外，它们只涵盖不到 20 个类别中的不到 100 个应用。尽管 AARSynth、Srisopha 等人和 PPrior 等数据集提供了包含数千个应用数百万条评论的更大数据集，但它们不适合构建有效的应用推荐系统，因为它们缺少唯一的用户标识符。此外，Srisopha 等人[38]的数据集不公开可用，而 PPrior 只提供负面用户评论。表 1 展示了移动应用数据集的比较。本文工作试图通过提供一个大规模、丰富且多样化的基准数据集来填补这一研究空白，我们称之为 MobileRec，以促进研究人员开发应用推荐系统。据我们所知，这是唯一一个提供每个用户与应用多次序列化交互的移动应用数据集。我们还在表 2 中展示了我们提出的数据集与不同领域知名推荐数据集最新版本的比较。

MobileRec 是一个基于用户在 Google Play 商店中的活动构建的大规模数据集，包含 48 个类别中超过 1 万个独立应用的 1930 万次用户交互（即用户的评分连同评论）。它记录了 70 万唯一用户的序列化活动，每个用户至少与 5 个应用交互。我们数据集中的每次用户交互都包含基本信息，例如用户对已安装应用的评分、文本评论和评论日期。此外，MobileRec 中的每个独立应用都配备了丰富的元数据，例如应用的标题、类别、长篇文本描述、总体平均评分、开发者信息、内容分级以及其他相关细节。表 3 展示了数据集的重要特征。为激发应用推荐系统的研究，MobileRec 与来自其他领域的成熟且广泛认可的数据集相当。除了应用推荐之外，我们全面的数据集还可以提供重要的见解，并作为多种研究和数据分析工作的宝贵资源，例如情感分析、应用开发和优化、市场研究和欺诈检测等。

具体来说，本文的贡献如下：

- 我们提出了 MobileRec，这是迄今为止最全面的序列化用户-应用交互集合，包含来自 Google Play 的跨越所有类别的超过 1 万个不同应用的 1900 多万次交互。值得注意的是，这是唯一一个每个用户具有多次交互的移动应用数据集。
- 我们的实验研究调查了用户-应用交互的动态性，并通过采用几种最先进的推荐系统并为研究社区建立基线结果，证明了 MobileRec 的实际效用。

我们还表明，通过对比评估几种最先进的推荐技术，MobileRec 可以作为应用推荐研究的一个有价值的实验测试平台。具体来说，我们使用 MobileRec 对几种现有的通用和序列推荐系统进行了评估，包括 Pop（即item的流行度）、SASRec[23]、ELECRec[7]、BERT4Rec[39]、HGN[28]、SINE[40]、LightSANs[12]、GRU4Rec[41]和 GCSAN[47]。我们对这些方法在 MobileRec 上获得的结果的分析揭示了一些有趣的趋势。值得注意的是，我们观察到 ELECRec[7] 在 Amazon-Beauty 数据集上的表现显著优于 GRU4Rec[41]，在 Hit@5 指标上实现了超过 300% 的提升。然而，当在 MobileRec 上测试时，ELECRec 在同一指标上与 GRU4Rec 相比出现了显著的性能下降。类似地，我们观察到 LightSANs[12] 与 SASRec[23] 相比，在 Hit@10 和 NDCG@10 指标上分别实现了 3.91% 和 2.65% 的提升。然而，在 MobileRec 上，SASRec[23] 优于 LightSANs[12]。这表明 MobileRec 数据集中用户-item交互和用户兴趣具有内在的动态性和短暂性，从而产生了复杂的用户交互历史。这一观察对最先进的推荐模型具有重要意义。本文提出的数值结果也可以作为其他研究人员评估其自身结果、客观评价不同方法质量和有效性的基线。展望未来，许多自然语言处理技术，如先进的文本表示方法和预训练语言模型[9, 35]，有潜力为使用 MobileRec 的应用推荐开辟新的研究方向。这些技术有望提高应用推荐系统的质量和有效性，因此特别引起研究社区的兴趣。
...
Yelp[4]数据集从 2018 年到 2022 年有多个版本。最新版本 Yelp2022 有 190 万唯一用户、15 万个不同item和 690 万条评论。类似地，MovieLens[1]提供 ML-25M[17]，包含 2500 万个评分和 100 万个标签应用于 62K 部电影。该数据集的先前版本包括 ML-100k 和 ML-1M。

除上述大规模数据集外，来自不同领域的各种数据集在推动各自领域的研究中发挥了重要作用。下面我们简要讨论其他推荐数据集。Steam 包含从 Steam 爬取的评论和游戏信息[23]。Book-Crossing[50]和 GoodReads[44, 45]提供用户-item交互，其中交互类型为评分。DIGINETICA[49]包含 20 万用户、184K item和约 100 万次交互，其中交互类型为用户点击。该数据集是从电子商务搜索引擎日志中提取的用户会话编译而成。Twitch[36]提供 1500 万用户在 600 万个item上的 4.74 亿次交互，交互类型为用户点击。这些数据集使得各自领域的多个推荐系统得以发展[10, 18, 21, 31, 33, 34, 37, 42, 48]。我们期望 MobileRec 发挥类似作用，激发构建鲁棒应用推荐系统的研究。
...
为确保数据质量，我们设置了多项检查以消除重复条目、错误信息以及在爬取过程中可能发生的其他错误。我们在进一步处理数据以转换为 5-core 之前移除了数据中的重复项。我们还通过引入 16 字符字母数字 uid 对用户进行了匿名化。我们只保留了那些有超过 5 条评论的用户。评论少于 15 条的item从最终数据集中移除。最终结果是一个大规模 5-core 的用户评论数据集（即带时间戳的交互），用于 Google Play 上的应用，提供了关于用户对各种应用的意见和体验的宝贵见解。表 1 和表 2 提供了关于数据集的重要统计信息。
...
| date | 特定用户/item交互的日期，即评论日期。示例：October 21, 2018, November 4, 2021, January 16, 2021 |
| formated_date | 特定用户/item交互的日期（YYYY-MM-DD 格式的评论日期）。示例：2018-10-21, 2021-11-04, 2021-01-16 |
...
**Pop.** 这是一个简单的基于流行度的模型。在此模型中，记录item的流行度，并将最流行的item推荐给用户。例如，在 YouTube 上使用此模型，将向用户推荐观看次数最多的视频。

### 4.2 序列推荐

**SASRec [23].** 马尔可夫链假设用户的下一个动作可以由他们最近的动作（仅一个或几个最近的动作）决定。另一方面，基于 RNN 的方法可以考虑长期用户-item交互历史来发现用户的隐藏兴趣。SASRec 通过采用注意力机制，将基于马尔可夫链（MC）的方法和基于循环神经网络（RNN）的方法的优点结合在一个统一的设计中。基于 MC 的方法在稀疏数据集的情况下有效，而基于 RNN 的方法适用于涉及更密集数据集的情况。SASRec 提出平衡基于 MC 的方法和基于 RNN 的方法的威力，以获得两全其美的效果。

**ELECRec [7].** 下一个item预测任务通常被建模为生成式任务。ELECRec 提出在推荐系统中使用判别器进行下一个item预测任务。判别器负责判断下一个采样item是否为真实的目标item。生成器 G 和判别器 D 联合训练。生成器 G 被训练为判别器生成高质量的样本。

**BERT4Rec [39].** 基于序列神经网络是次优且有限的这一观察。这种次优性可归因于对用户行为的限制性单向（从左到右）编码，以及他们假设用户交互序列是有序的。BERT4Rec 是一种序列推荐模型，它采用双向自注意力来编码用户的交互序列。该基线将完形填空目标应用于序列推荐，并通过双向上下文联合条件来预测被掩码的item。

**HGN [28].** 考虑到近期时间顺序用户-item交互的重要性，层次门控网络提出了一种通过集成贝叶斯个性化排序（BPR）来捕捉长期和短期用户兴趣的方法。所提出的方法包括特征门控模块、item门控模块和item-item乘积模块。门控模块负责决定哪些特征从特征层和实例层转发到下游层。item-item乘积模块捕捉用户过去交互过的item和将来将交互的item之间的item关系。

**SINE [40].** 认识到用户行为序列中多个概念上不同item的重要性，SINE 提出捕捉用户行为序列的统一嵌入主要受最近用户交互的影响。基于这一观察，SINE 提出使用多个嵌入来捕捉用户交互行为的各个方面。由于概念池可能很大，SINE 能够从大型概念池中推断出稀疏的概念集。拥有多个嵌入后，采用兴趣聚合模块来预测用户当前的意图，这也用于建模下一个item预测。

### 4.3 基于 Transformer 的推荐

**LightSANs [12].** 自注意力网络（SAN）由于二次复杂度和对过参数化的脆弱性而受限。序列关系中item间的建模不准确性也是 SAN 的一个限制因素，因为隐式位置编码。为此，LightSAN 提出了一种低秩分解的自注意力网络。低秩分解的自注意力将用户的历史item投影到几个潜在兴趣上。item到兴趣的交互用于生成上下文感知的表示。
...
对于序列推荐基线，我们保持一致批处理大小为 4096，最大交互长度为 50。我们采用留一法进行验证和测试，并使用完整item集进行评估。我们使用早停 patience 为 10。我们将序列推荐任务视为多分类任务，并使用交叉熵损失来训练模型。SASRec[23]使用 adam 优化器和 0.001 的学习率训练。层数和注意力头数为 2。dropout 率为 0.5，使用 gelu 作为激活函数。HGN[28]和 SINE[40]使用嵌入大小 64、学习率 0.001 和 adam 优化器训练。LightSANs[12]的潜在兴趣维度为 5，2 个注意力头数和 2 个 Transformer 层。训练使用学习率 0.001 和 adam 优化器。GCSAN[47]有 2 个 Transformer 编码器层、2 个注意力头数、隐藏状态特征大小为 64、前馈层隐藏大小为 256、权重设为 0.6、图神经网络层数为 1，使用 adam 优化，学习率为 0.001。GRU4Rec[41]使用 1 层训练，嵌入大小为 64、隐藏大小为 128、dropout 为 0.3。我们使用 RecBole[49]为 MobileRec 建立基线。

### 5.2 评估指标

我们采用标准指标 Hit@K 和 NDCG@K，其中 $K \in \{1, 5, 10, 15, 20\}$，作为基准方法的评估指标。Hit@k 考虑预测item出现在 top-K 列表中的次数，可表示为[27]中的形式：

$$
\mathrm{HR@K} = \frac{1}{M}\sum_{u=1}^{M}\mathbb{1}\{R_u \le K\} = \sum_{R=1}^{N} W_R \cdot \mathbb{1}\{R \le K\}
$$

$$
W_R = \frac{1}{M}\sum_{u=1}^{M}\mathbb{1}\{R_u = R\}
$$

其中 $\mathbb{1}_X$ 表示示性随机变量， $M$ 是用户总数， $N = |I|$ 是总item数。 $R$ 是item在范围 $[1, N]$ 内的整数排名位置。 $R_u$ 是item $i_u$ 在 $I$ 个item中对用户 $u$ 的排名。 $W_R$ 捕捉在位置 $R$ 上拥有item $i_u$ 的用户。

NDCG@K 可表示为[22, 49]：

$$
\mathrm{NDCG@K} = \frac{1}{|U|}\sum_{u \in U}\frac{\sum_{i=1}^{K}\frac{\delta(i \in R(u))}{\log_2(i+1)}}{\sum_{i=1}^{\min(|R(u)|,K)}\frac{1}{\log_2(i+1)}}
$$
...
表 4 展示了各种基线在 MobileRec 上的性能分析。接下来，我们讨论这些结果并提供更多细节。如前所述，Pop 是一个基于流行度的模型，依赖于item的流行度。Pop 是最朴素的模型，正如预期，其性能几乎逊于所有其他基线。SASRec 之前报告过根据 Hit@10 和 NDCG@10 性能指标，其结果优于 Pop。在 SASRec 中，随机采样了 100 个负样本item，并针对这 101 个item（包括 ground truth）计算 Hit@10 和 NDCG@10。使用 MobileRec，我们采用完整item集进行评估，这是推荐系统更严格的评估标准。我们观察到，在所有性能指标上，SASRec 均优于 Pop，除了 Hit@1。例如，与 Pop 相比，SASRec 的 Hit@10 提升了 19.86%。同样，Hit@15 观察到了 16.34% 的性能提升。类似地，在 MobileRec 上，SASRec 相比 Pop 的 Hit@20 提升了 15.23%。

考虑 NDCG 指标，SASRec 在 Beauty 数据集上评估时，相比 Pop 的 NDCG@10 提升了 41.37%。在 MobileRec 上，我们观察到相比 Pop，NDCG@10 仅提升了 14.28%。我们认为，SASRec 和 Pop 在这两个数据集（Beauty 和 MobileRec）上 NDCG@10 提升的差异可以通过比较 Beauty 和 MobileRec 的数据集统计信息来解释。Beauty 有 52,024 个用户，而 MobileRec 有 700,111 个用户。其次，Beauty 数据集有 57,289 个item，而 MobileRec 有 10,173 个应用。Beauty 有 40 万次交互，平均每个用户 7.6 次交互，每个item 6.9 次交互。而 MobileRec 有 1930 万次交互，平均每个用户 27.56 次交互，每个item（即应用）1,896.88 次交互。例如，MobileRec 每个用户有 27.56 次交互，是 Beauty 每用户交互次数的 200 倍以上。同样，MobileRec 每个item的平均交互次数也远多于 Beauty。考虑到这些因素，MobileRec 呈现了一个更加动态的推荐场景。MobileRec 中用户和item交互的高程度可能是 SASRec 相比 Pop 在 NDCG@10 上改进相对较小的原因。其次，由于我们使用完整item集进行评估，而 SASRec 使用排名策略，这也可能是 SASRec 相比 Pop 性能增益较小的潜在原因。在 Beauty 数据集上，SASRec 报告了比 GRU4Rec 更好的结果。我们在 MobileRec 上训练和评估时，也观察到 SASRec 优于 GRU4Rec 的相同模式。

在 Beauty 数据集上，ELECRec 报告的 Hit@5、Hit@10、NDCG@5 和 NDCG@10 优于 Pop。我们在 MobileRec 上训练和评估时也注意到相同的模式，唯一的例外是 Hit@1，其中 Pop 表现优于 ELECRec。我们推测非常动态的用户-item交互历史可能是原因。类似地，在 Beauty 数据集上，ELECRec 相比 GRU4Rec 报告的 Hit@5 提升了 329.87%，Hit@10 提升了 242.04%。此外，对于 NDCG@5 和 NDCG@10 指标，ELECRec 相比 GRU4Rec 分别达到了 412.12% 和 331.38% 的提升。着眼于在 MobileRec 上训练和评估时 ELECRec 相对于 GRU4Rec 的性能提升，ELECRec 在 Hit@5 和 Hit@10 上分别实现了 9.30% 和 13.72% 的提升。类似地，在 NDCG@5 和 NDCG@10 上分别观察到 1.88% 和 10.81% 的提升。这一观察也指向了 MobileRec 中高度动态的用户-item交互。两种模型都难以实现高性能，这导致两种方法之间的性能差距缩小，相比于 ELECRec 中报告的性能差距。MobileRec 中有超过 1900 万次交互，而 Beauty 中只有 40 万次用户-item交互。用户-item交互的这种高度动态性将模型推向较低的性能增益。考虑到这一分析，MobileRec 应成为现有推荐数据集的重要补充。

ELECRec 在 Beauty 数据集上也展示了相对于 SASRec 的比较增益。首先，我们将介绍 ELECRec 在 Beauty 数据集上相对于 SASRec 在 Hit@5、Hit@10、NDCG@5 和 NDCG@10 方面的相对性能增益。之后，我们将讨论当训练和评估数据集为 MobileRec 时，ELECRec 相对于 SASRec 实现的性能改进。从 Hit@5 和 Hit@10 开始，ELECRec 相比 SASRec 分别提升了 83.59% 和 59.47%。类似地，ELECRec 相比 SASRec 在 NDCG@5 和 NDCG@10 方面分别报告了 103.61% 和 84.11% 的提升。现在，让我们讨论在 MobileRec 上训练和评估时 ELECRec 和 SASRec 之间的性能比较。可以观察到，SASRec 在 MobileRec 上的 Hit@5、Hit@10、NDCG@5 和 NDCG@10 指标上表现优于 ELECRec。SASRec 相对于 ELECRec 在 Hit@5、Hit@10、NDCG@5 和 NDCG@10 方面的性能改进量化分别为 4.25%、4.02%、8.92% 和 7.31%。我们认为 ELECRec 使用了一个用 NLP 任务训练的生成器，而判别器负责判断item是否是序列中正确的下一个（真实）item或虚假的下一个item。判别器的判别能力取决于生成器生成的样本质量。由于 MobileRec 呈现出高度动态的用户-item交互序列，且用户兴趣转瞬即逝，生成器可能难以生成高质量的训练样本。由于判别器捕捉真实item相关性的能力取决于生成器生成的样本质量，低质量样本可能导致判别器部分性能增益下降。

值得回顾图 2，它提供了动态用户-item交互序列的有趣快照。例如，从图 2(b)可以看出，相当大比例的用户从付费应用迁移到免费应用。类似的动态迁移模式在图 2(d)中也很明显，该图描绘了用户在 top-5 类别中的迁移。我们认为，具有如此动态时间交互模式的 1900 万用户-item交互可能对生成器学习高质量样本生成构成挑战。此外，ELECRec 报告了其与 BERT4Rec 的比较性能。在原始论文中，ELECRec 在 Beauty 数据集的所有指标上都优于 BERT4Rec[7]。具体而言，ELECRec 在 Hit@5 和 Hit@10 上相比 BERT4Rec 分别报告了 100.85% 和 61.06% 的性能增益，在 NDCG@5 和 NDCG@10 指标上分别报告了 131.50% 和 97% 的增益。我们观察到当使用 MobileRec 进行训练和评估时，ELECRec 在大多数评估指标上优于 BERT4Rec 的相同模式，除了 Hit@1，BERT4Rec 优于 ELECRec。考虑到 Hit@1 是更严格的标准，我们认为 BERT4Rec 采用的通过双向上下文联合条件预测序列中被掩码item的训练目标，对于像 Hit@1 这样更严格的评估指标可能是更好的训练目标。尤其是在像 MobileRec 这样用户兴趣转瞬即逝的背景下，使用双向上下文联合条件的掩码item预测目标似乎在编码用户动态行为方面有效。
...
LightSANs 是一种用于下一个item推荐任务的 Transformer 变体，它采用低秩分解的自注意力将用户的历史兴趣投影到几个潜在兴趣上。LightSANs 在多个竞争基线上报告了 Hit@10 和 NDCG@10 结果。我们将重点放在 Pop、GRU4Rec、BERT4Rec 和 SASRec 上，以比较和分析 LightSANs 在 Amazon-Books 数据集和 MobileRec 上的性能。在进一步深入比较分析 LightSANs 在 Amazon-Beauty 与 MobileRec 上的性能之前，让我们先看 Amazon-Books 数据集。Amazon-Books 数据集有 19K 用户、60K item和 170 万次交互。在 Amazon-Books 数据集上，LightSANs 相比 Pop 在 Hit@10 和 NDCG@10 上分别报告了 121.77% 和 172.43% 的提升。类似地，相比 SASRec，在 Hit@10 和 NDCG@10 上分别报告了 3.91% 和 2.65% 的提升。相比 GRU4Rec，在 Hit@10 和 NDCG@10 上分别报告了 8.41% 和 5.72% 的提升。最后，相比 BERT4Rec，报告了 8.76% 和 4.03% 的提升。当数据集为 MobileRec 时，我们观察到相比 Pop，在 Hit@10 和 NDCG@10 上分别有 13.90% 和 10.38% 的提升。LightSANs 相比 GRU4Rec 在同一指标上分别获得了 12.41% 和 14.86% 的提升。

与 ELECRec 类似，LightSANs 相比 BERT4Rec 在 Hit@10 和 NDCG@10 指标上也展示了 22.85% 和 18.05% 的更好结果。我们注意到 LightSANs 在 MobileRec 上保持了优于竞争基线的模式，就像在 Amazon-Books 数据集上一样，但 SASRec 是一个例外，它在 Hit@10 和 NDCG@10 指标上优于 LightSANs。LightSANs 相对于 BERT4Rec、Pop 和 GRU4Rec 表现出的稳定性能可归因于 LightSANs 中更好的设计选择。例如，LightSANs 提出了解耦位置编码来代替隐式位置编码。我们相信这种解耦位置编码有助于 LightSANs 更精确地建模历史用户兴趣和用户-item交互，从而引导模型在 MobileRec 上获得更好的 Hit@10 和 NDCG@10。尽管如此，LightSANs 被 SASRec 超越。SASRec 的优越性能可能归因于 SASRec 引入的可学习位置嵌入。
...
| 至少 15 次交互的item（千） | 2072 | 77.58 | 20.59 | 10.17 |
| 单个用户的最大交互数 | 446 | 3048 | 32202 | 256 |
| 单个用户的最小交互数 | 1 | 1 | 20 | 5 |
| 每用户平均交互数 | 5.32 | 3.52 | 153.81 | 27.56 |
| 单个item的最大交互数 | 13,560 | 7,673 | 81,491 | 14,345 |
| 单个item的最小交互数 | 1 | 5 | 1 | 20 |
| 每item平均交互数 | 15.45 | 46.49 | 423.39 | 1896.88 |
