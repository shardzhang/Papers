...
**CCS概念：** • 计算方法 → 多任务学习；神经网络；• 信息系统 → 推荐系统
...
最后，我们在一个真实的大规模内容推荐系统上测试了MMoE，其中两个分类任务同时在向用户推荐物品时进行学习。我们使用数千亿的训练样本训练MMoE模型，并将其与共享底层生产模型进行比较。我们观察到离线指标（如AUC）有显著改进。此外，我们的MMoE模型在在线实验中持续改进在线指标。

本文的贡献有三方面：首先，我们提出了一种新颖的多门混合专家模型，它显式建模任务关系。通过调制和门控网络，我们的模型自动调整建模共享信息和建模任务特定信息之间的参数化。其次，我们在合成数据上进行了控制实验。我们报告了任务相关性如何影响多任务学习中的训练动态，以及MMoE如何提高模型表达能力和可训练性。最后，我们在真实基准数据和一个拥有数亿用户和物品的大规模生产推荐系统上进行了实验。我们的实验验证了我们提出的方法在真实环境中的效率和有效性。
...
(1) 给定输入特征维度d，我们生成两个正交单位向量u1, u2 ∈ Rd，即 u1ᵀu2 = 0, ||u1||₂ = 1, ||u2||₂ = 1。

(2) 给定一个比例常数c和一个相关性分数−1 ≤ p ≤ 1，生成两个权重向量w1, w2使得：
    w1 = cu1, w2 = c(pu1 + √(1−p²)u2) (2)

(3) 随机采样一个输入数据点x ∈ Rd，其每个元素来自N(0, 1)。

(4) 为两个回归任务生成两个标签y1, y2如下：
    y1 = w1ᵀx + Σᵢ₌₁ᵐ sin(αᵢw1ᵀx + βᵢ) + ε₁ (3)
    y2 = w2ᵀx + Σᵢ₌₁ᵐ sin(αᵢw2ᵀx + βᵢ) + ε₂ (4)
    其中αᵢ, βᵢ, i = 1, 2, ..., m是控制正弦函数形状的给定参数，ε₁, ε₂ ~ N(0, 0.01)

(5) 重复步骤(3)和(4)直到生成足够的数据。

由于非线性数据生成过程，直接生成具有给定标签Pearson相关性的任务并不简单。相反，我们操作公式2中权重向量的余弦相似度，即cos(w1, w2) = p，然后测量结果标签的Pearson相关性。注意在线性情况下，y1 = w1ᵀx + ε₁, y2 = w2ᵀx + ε₂，y1, y2的标签Pearson相关性正好是p。在非线性情况下，公式3和公式4中的y1和y2也是正相关的，如图2所示。在本文的其余部分，为简单起见，我们将权重向量的余弦相似度称为"任务相关性"。
...
其中 Σᵢ₌₁ⁿ g(x)ᵢ = 1，g(x)ᵢ是g(x)输出的第i个logit，表示专家fᵢ的概率。
...
其中W_gᵏ ∈ Rⁿˣᵈ是一个可训练的矩阵，n是专家数量，d是特征维度。
...
**模型结构：** 输入维度为100。两种基于MoE的模型都有8个专家，每个专家实现为单层网络。专家网络中隐藏层的大小为16。塔网络仍然是大小为8的单层网络。我们注意到，共享专家和塔中的模型参数总数为100 × 16 × 8 + 16 × 8 × 2 = 13056。对于基线共享底层模型，我们仍然将塔网络设置为大小为8的单层网络。我们将单层共享底层网络的大小设置为13056/(100 + 8×2) ≈ 113。
...
给定yᵏ作为任务k（k∈{1,2}）的真实标签，任务k的预测表示为ŷᵏ = f(x; θᵏ)，其中θᵏ是模型参数。

该方法的损失函数为：L(y¹, f(x; θ¹)) + L(y², f(x; θ²)) + α||θ¹−θ²||₂²，其中y¹, y²是任务1和任务2的真实标签，α是一个超参数。该方法通过α的大小来建模任务相关性。

**Cross-Stitch[27]：** 该方法通过引入"Cross-Stitch"单元在两个任务之间共享知识。Cross-Stitch单元接收来自任务1和2的分离隐藏层的输入x¹和x²，并通过以下方程输出x̃¹和x̃²：

[x̃¹; x̃²] = [α₁₁ α₁₂; α₂₁ α₂₂] [x¹; x²]

其中αⱼᵏ, j,k=1,2是可训练参数，表示从任务k到任务j的交叉迁移。x̃¹和x̃²分别被发送到任务1和任务2的更高级别层。

**张量分解[34]：** 在该方法中，来自多个任务的权重被建模为张量，并使用张量分解方法进行跨任务参数共享。在我们的比较中，我们实现了用于学习多任务模型的Tucker分解，据报道它提供了最可靠的结果[34]。例如，给定输入隐藏层大小m、输出隐藏层大小n和任务数量k，权重W（一个m×n×k张量）由以下方程得到：

W = Σᵢ₁ Σᵢ₂ Σᵢ₃ S(i₁,i₂,i₃) · U₁(:,i₁) ◦ U₂(:,i₂) ◦ U₃(:,i₃)

其中大小为r₁×r₂×r₃的张量S、大小为m×r₁的矩阵U₁、大小为n×r₂的矩阵U₂和大小为k×r₃的矩阵U₃是可训练参数。它们都通过标准反向传播一起训练。r₁, r₂, r₃是超参数。

### 6.2 超参数调优

我们采用一个超参数调优器（用于最近的深度学习框架[10]），来搜索真实数据集实验中所有模型的最佳超参数。调优算法是一个高斯过程模型，类似于Spearmint，如[14, 32]中所述。

为了使比较公平，我们通过设置每层隐藏单元数的相同上限（2048）来约束所有方法的最大模型大小。对于MMoE，它是"专家数×每个专家隐藏单元数"。我们的方法和所有基线方法都使用TensorFlow[1]实现。
...
在本节中，我们在Google Inc.的一个大规模内容推荐系统上进行实验，该系统的推荐来自数亿个独特物品，面向数十亿用户。具体来说，给定用户当前消费某个物品的行为，该推荐系统旨在向用户展示一个相关物品列表供下一步消费。
