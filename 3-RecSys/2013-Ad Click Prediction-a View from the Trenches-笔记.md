...
FTRL：主要三点改进，一是考虑历史累计梯度的学习率自适应，对高频特征使用小学习率缓慢更新，对低频特征使用大学习率快速更新。同时使用近端正则化项（包括了L2正则化项），防止梯度出现漂移，稳住参数更新方向。三是使用L1正则进行稀疏化。L2正则转移到计算历史累计梯度，对于单个样本的梯度波动具有较好平滑效果，重要特征历史累计梯度较大，不会被L1正则置零。不重要特征通过L1正则置零。L1和L2职责拆分，L1负责参数置零，L2负责保护重要特征。因此在稀疏性和精度上都有较好的平衡。





2023年KDD最佳论文。这篇论文是2013年KDD最佳论文，来自Google的工程团队，分享了广告点击率（CTR）预测系统在实际部署中的经验教训。下面按章节详细解读：
...
Q：在线梯度下降的标准理论建议使用全局学习率调度 $\eta_t = 1/\sqrt{t}$，这**对所有坐标都是通用的** [34]。 
...
Q：其中我们**根据学习率调度定义 $\sigma_s$，使得 $\sigma_{1:t} = 1/\eta_t$**。
