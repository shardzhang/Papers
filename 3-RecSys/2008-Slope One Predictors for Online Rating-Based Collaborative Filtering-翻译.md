本文分享了Slope One系列协同过滤预测器，该系列算法基于`f(x) = x + b`形式的线性回归模型，通过预先计算物品间的平均评分差异来进行评分预测。核心内容：
- 提出了三种Slope One方案：基本Slope One、加权Slope One和双极Slope One，这些方案简单、易实现、支持在线查询和动态更新
- 通过将用户喜欢的物品与不喜欢的物品分开处理，双极Slope One方案进一步提升了预测精度
...
基于评分的协同过滤（Rating-based Collaborative Filtering）是根据其他用户的评分来预测某个用户将如何评价给定物品的过程。我们提出了三种相关的Slope One方案，其预测器形式为 `f(x) = x + b`，这些方案预先计算了同时对两个物品进行评分的用户对这两个物品评分之间的平均差异。Slope One算法易于实现、查询效率高、精度合理，并且支持在线查询和动态更新，这使其成为现实世界系统的理想候选。我们建议将基本的SLOPE ONE方案作为协同过滤的新参考方案。通过将用户喜欢的物品与用户不喜欢的物品分开处理，我们获得了与在标准基准数据集EachMovie和MovieLens上速度较慢的记忆基方案相竞争的结果，同时更好地满足了协同过滤应用的各项需求。

**关键词：** 协同过滤，推荐系统，电子商务，数据挖掘，知识发现

## 1 引言

在线基于评分的协同过滤（CF）查询由单个用户的一组（物品，评分）对组成。对该查询的响应是一组预测的（物品，评分）对，用于该用户尚未评分的物品。我们的目标是提供稳健的CF方案，这些方案应满足：
...
我们的Slope One算法基于一个直观原则——用户对物品间的"流行度差异"。我们以成对方式确定一个物品比另一个物品更受欢迎的程度。衡量这种差异的一种方法就是简单地计算两个物品的平均评分之差。反过来，这种差异可用于根据用户对另一个物品的评分来预测该用户对其中一个物品的评分。考虑两个用户A和B、两个物品I和J以及图1。用户A给物品I的评分为1，用户B给物品I的评分为2，而用户A给物品J的评分为1.5。我们观察到物品J比物品I高1.5 - 1 = 0.5分，因此我们可以预测用户B将给物品J打2 + 0.5 = 2.5分。我们称用户B为被预测用户，物品J为被预测物品。训练集中对每个未知评分存在许多这样的差异，我们取这些差异的平均值。本文提出的Slope One方案系列源于我们选择相关差异以获得单一预测的三种方式。
...
我们可以用以下代数形式将我们的预测器与文献中描述的某些预测器进行比较。我们的预测器形式为 `f(x) = x + b`，因此得名"slope one"（斜率为一），其中b是常数，x是表示评分值的变量。对于任意一对物品，我们试图找到最佳函数f，使其能够根据一个物品的评分预测另一个物品的评分。这个函数对于每对物品可能不同。一个CF方案将对预测器生成的多个预测进行加权。在[14]中，作者考虑了物品对之间的相关性，然后推导出用户评分的加权平均值作为预测器。在他们算法的简单版本中，预测器形式为 `f(x) = x`。在基于回归的版本中，预测器形式为 `f(x) = ax + b`。在[17]中，作者也采用了 `f(x) = ax + b` 形式的预测器。这两篇论文工作的一个自然扩展是考虑 `f(x) = ax² + bx + c` 形式的预测器。然而，在本文中，我们使用 `f(x) = x + b` 形式的朴素预测器。我们也使用朴素加权。[14]中观察到，即使他们基于回归的 `f(x) = ax + b` 算法也没有带来相对于记忆基算法的显著改进。因此，我们证明 `f(x) = x + b` 形式的预测器可以与记忆基方案竞争，这本身就是一个重要结果。

## 3 CF算法

我们提出了三种新的CF方案，并将我们的方案与四种参考方案进行对比：PER USER AVERAGE（用户平均）、BIAS FROM MEAN（均值偏差）、ADJUSTED COSINE ITEM-BASED（调整余弦物品基，一种模型基方案）以及PEARSON方案（记忆基方案的代表）。

### 3.1 符号说明

在描述方案时，我们使用以下符号。来自某个用户的评分（称为一个评价）表示为一个不完整的数组u，其中uᵢ是该用户对物品i的评分。在u中被评分物品的子集表示为S(u)。训练集中所有评价的集合为χ。集合S中的元素数量为card(S)。评价u中评分的平均值记为ū。集合Sᵢ(χ)是所有包含物品i（i ∈ S(u)）的评价u ∈ χ的集合。给定两个评价u和v，我们定义标量积⟨u, v⟩为∑ᵢ∈S(u)∩S(v) uᵢvᵢ。预测结果P(u)表示一个向量，其中每个分量对应一个物品的预测值：预测结果隐式地依赖于训练集χ。

### 3.2 基线方案

最基本的预测算法之一是PER USER AVERAGE（用户平均）方案，其公式为 `P(u) = ū`。即，我们预测用户将根据其平均评分来评价所有物品。

另一个简单方案称为BIAS FROM MEAN（均值偏差，有时也称为NON PERSONALIZED[8]），其公式为：

`P(u)ᵢ = ū + (1 / card(Sᵢ(χ))) * Σᵥ∈Sᵢ(χ) (vᵢ - v̄)`

即，该预测基于用户平均评分加上训练集中所有用户对特定物品与其自身平均值的平均偏差。

我们还将与效果最好的物品基方法[14]进行比较，该方法使用以下调整余弦相似度度量，给定两个物品i和j：

`simᵢⱼ = (Σᵤ∈Sᵢⱼ(χ) (uᵢ - ū)(uⱼ - ū)) / sqrt(Σᵤ∈Sᵢⱼ(χ) (uᵢ - ū)² * Σᵤ∈Sᵢⱼ(χ) (uⱼ - ū)²)`

预测结果通过以下加权和获得：

`P(u)ᵢ = (Σⱼ∈S(u) |simᵢⱼ|(αᵢⱼuⱼ + βᵢⱼ)) / (Σⱼ∈S(u) |simᵢⱼ|)`

其中回归系数αᵢⱼ、βᵢⱼ通过最小化Σᵤ∈Sᵢⱼ(u)(αᵢⱼuⱼ + βᵢⱼ - uᵢ)² 来选取，其中i和j固定。

### 3.3 PEARSON参考方案

由于我们希望证明我们的方案在预测能力上与记忆基方案相当，我们选择实现一个此类方案作为该类别的代表，并承认存在许多该类型的已有文献记载的方案。在最流行和精确的记忆基方案中，有PEARSON方案[13]。它采用对χ中所有用户的加权和形式：

`P(u)ᵢ = ū + (Σᵥ∈Sᵢ(χ) γ(u, v)(vᵢ - v̄)) / (Σᵥ∈Sᵢ(χ) |γ(u, v)|)`

其中γ是根据皮尔逊相关性计算的相似度度量：

`Corr(u, w) = ⟨u - ū, w - w̄⟩ / sqrt(Σᵢ∈S(u)∩S(w) (uᵢ - ū)² * Σᵢ∈S(u)∩S(w) (wᵢ - w̄)²)`

按照[2, 8]的做法，我们设定：

`γ(u, w) = Corr(u, w) * |Corr(u, w)|^(ρ-1)`

其中ρ = 2.5，ρ是案例放大（Case Amplification）的幂次。案例放大减少了数据中的噪声：如果相关性高，例如Corr = 0.9，那么经过案例放大后仍然较高（0.9^2.5 ≈ 0.8）；而如果相关性低，例如Corr = 0.1，那么它就变得可忽略不计（0.1^2.5 ≈ 0.003）。[2]表明，皮尔逊相关性结合案例放大是一种精度合理的记忆基CF方案，尽管存在更精确的方案。

### 3.4 SLOPE ONE方案

Slope One方案同时考虑了来自其他对同一物品评分的用户的信息（类似于ADJUSTED COSINE ITEM-BASED）和来自同一用户评分的其他物品的信息（类似于PER USER AVERAGE）。然而，该方案也依赖于既不属于用户数组也不属于物品数组的数据点（例如图1中用户A对物品I的评分），但这些数据点对于评分预测而言仍然是重要信息。该方法的大部分优势来自于那些未被单独纳入的数据。具体而言，只有那些与待预测用户共同评价过某个物品的用户评分，以及只有那些待预测用户也已评分的物品评分，才会进入Slope One方案下的评分预测。

形式化地，给定两个评价数组vᵢ和wᵢ（其中i = 1, …, n），我们寻找最佳形式的预测器 `f(x) = x + b` 来从v预测w，通过最小化 Σᵢ(vᵢ + b - wᵢ)²。对b求导并令导数为零，得到 `b = (Σᵢ(wᵢ - vᵢ)) / n`。换句话说，常数b必须被选为两个数组之间的平均差异。这一结果启发了以下方案。

给定训练集χ以及任意两个物品j和i，它们在某些用户评价u中分别具有评分uⱼ和uᵢ（记为 u ∈ Sⱼᵢ(χ)），我们考虑物品i相对于物品j的平均偏差为：

`devⱼᵢ = Σᵤ∈Sⱼᵢ(χ) (uⱼ - uᵢ) / card(Sⱼᵢ(χ))`

注意，任何不同时包含uⱼ和uᵢ的用户评价u都不会被纳入求和。由devⱼᵢ定义的对称矩阵可以一次性计算完毕，并在新数据输入时快速更新。

鉴于 `devⱼᵢ + uᵢ` 是根据uᵢ对uⱼ的一个预测，一个合理的预测器可能是所有此类预测的平均值：

`P(u)ⱼ = (1 / card(Rⱼ)) * Σᵢ∈Rⱼ (devⱼᵢ + uᵢ)`

其中 `Rⱼ = {i | i ∈ S(u), i ≠ j, card(Sⱼᵢ(χ)) > 0}` 是所有相关物品的集合。有一种近似方法可以简化此预测的计算。对于一个足够密集的数据集，其中几乎所有物品对都有评分（即对于几乎所有i, j，都有card(Sⱼᵢ(χ)) > 0），大多数情况下，对于j ∉ S(u)有 `Rⱼ = S(u)`，而当j ∈ S(u)时有 `Rⱼ = S(u) - {j}`。由于对于大多数j，`ū = (Σᵢ∈S(u) uᵢ) / card(S(u)) ≈ (Σᵢ∈Rⱼ uᵢ) / card(Rⱼ)`，我们可以将SLOPE ONE方案的预测公式简化为：

`P^S1(u)ⱼ = ū + (1 / card(Rⱼ)) * Σᵢ∈Rⱼ devⱼᵢ`

有趣的是，我们实现的SLOPE ONE并不依赖于用户如何评价单个物品，而仅依赖于用户的平均评分以及（至关重要的）用户评价了哪些物品。

### 3.5 加权SLOPE ONE方案

SLOPE ONE的一个缺点是没有考虑观测到的评分数量。直观地说，要预测用户A对物品L的评分（已知用户A对物品J和K的评分），如果有2000个用户对物品对(J, L)进行了评分，而只有20个用户对物品对(K, L)进行了评分，那么用户A对物品J的评分很可能比用户A对物品K的评分更适合作为物品L的预测器。因此，我们将加权SLOPE ONE预测定义为以下加权平均：

`P^wS1(u)ⱼ = (Σᵢ∈S(u)-{j} (devⱼᵢ + uᵢ) * cⱼᵢ) / (Σᵢ∈S(u)-{j} cⱼᵢ)`

其中 `cⱼᵢ = card(Sⱼᵢ(χ))`。

### 3.6 双极SLOPE ONE方案

加权方案倾向于频繁出现的评分模式而非不频繁的评分模式，现在我们考虑另一种特别相关的评分模式。我们通过将预测分成两部分来实现这一点。使用加权SLOPE ONE算法，我们根据用户喜欢的物品推导出一个预测，再根据用户不喜欢的物品推导出另一个预测。

给定一个评分范围（比如0到10），使用范围中值5作为阈值，将高于5分的物品视为喜欢、低于5分的视为不喜欢似乎是合理的。如果用户的评分分布均匀，这种方法效果很好。然而，EachMovie数据集中超过70%的评分高于范围中值。由于我们希望支持所有类型的用户（包括均衡型、乐观型、悲观型和双峰型用户），我们使用用户的平均评分作为用户喜欢和不喜欢物品之间的阈值。例如，乐观型用户喜欢他们评价的每一件物品，我们假设他们不喜欢低于其平均评分的物品。这个阈值确保我们的算法对每个用户都有合理数量的喜欢和不喜欢物品。

再次参见图1，与往常一样，我们根据同时对物品I和J评分的用户（如用户A）对物品I的偏差，来预测用户B对物品J的评分。双极SLOPE ONE方案进一步限制了用于预测的评分集合。首先，在物品方面，只考虑两个喜欢物品之间的偏差或两个不喜欢物品之间的偏差。其次，在用户方面，只有那些同时对物品I和J进行了评分并且对物品I有着相同喜欢或不喜欢的用户对的偏差，才被用于预测物品J的评分。

将每个用户拆分为用户喜欢和用户不喜欢，实际上使用户数量增加了一倍。然而，请注意，上述双极限制必然会减少预测计算中的评分总数。尽管在数据稀疏性问题下这种减少似乎会提高精度（这似乎违反直觉），但未能过滤掉不相关的评分可能带来更大的问题。关键在于，双极SLOPE ONE方案不会根据"用户A喜欢物品K而用户B不喜欢同一物品K"这一事实进行预测。

形式化地，我们将每个评价u拆分为两个已评分物品集合：`S^like(u) = {i ∈ S(u) | uᵢ > ū}` 和 `S^dislike(u) = {i ∈ S(u) | uᵢ < ū}`。对于每对物品i, j，将所有评价χ的集合拆分为 `S^likeᵢⱼ = {u ∈ χ | i, j ∈ S^like(u)}` 和 `S^dislikeᵢⱼ = {u ∈ χ | i, j ∈ S^dislike(u)}`。使用这两个集合，我们计算喜欢物品的偏差矩阵以及不喜欢物品的偏差矩阵：

`dev^likeⱼᵢ = Σᵤ∈S^likeⱼᵢ(χ) (uⱼ - uᵢ) / card(S^likeⱼᵢ(χ))`

`dev^dislikeⱼᵢ = Σᵤ∈S^dislikeⱼᵢ(χ) (uⱼ - uᵢ) / card(S^dislikeⱼᵢ(χ))`

根据i属于 `S^like(u)` 还是 `S^dislike(u)`，基于物品i的评分对物品j的预测分别为 `p^likeⱼᵢ = dev^likeⱼᵢ + uᵢ` 或 `p^dislikeⱼᵢ = dev^dislikeⱼᵢ + uᵢ`。双极SLOPE ONE方案由以下公式给出：

`P^bpS1(u)ⱼ = (Σᵢ∈S^like(u)-{j} p^likeⱼᵢ * c^likeⱼᵢ + Σᵢ∈S^dislike(u)-{j} p^dislikeⱼᵢ * c^dislikeⱼᵢ) / (Σᵢ∈S^like(u)-{j} c^likeⱼᵢ + Σᵢ∈S^dislike(u)-{j} c^dislikeⱼᵢ)`

其中权重 `c^likeⱼᵢ = card(S^likeⱼᵢ(χ))` 和 `c^dislikeⱼᵢ = card(S^dislikeⱼᵢ(χ))` 与加权SLOPE ONE方案中的权重类似。

## 4 实验结果

给定CF算法的有效性可以被精确度量。为此，我们采用了留一法平均绝对误差（All But One Mean Average Error，MAE）[2]。在计算MAE时，我们依次从测试集中的所有评价中隐藏单个评分，预测该隐藏评分，然后计算预测的平均误差。给定预测器P和来自用户的评价u，P在一组评价χ'上的误差率由以下公式给出：

`MAE = (1 / card(χ')) * Σᵤ∈χ' ((1 / card(S(u))) * Σᵢ∈S(u) |P(u^(i)) - uᵢ|)`

其中 `u^(i)` 是隐藏了用户对第i个物品评分uᵢ后的用户评价u。

我们在Compaq Research提供的EachMovie数据集和明尼苏达大学Grouplens Research Group提供的MovieLens数据集上测试了我们的方案。这些数据来自电影评分网站，EachMovie的评分范围为0.0到1.0，步长为0.2；MovieLens的评分范围为1到5，步长为1。按照[8, 11]的做法，我们使用足够多的评价来构成总量为50,000个评分的训练集(χ)，并使用额外的一组评价构成总量至少100,000个评分的测试集(χ')。当预测值超出给定数据集的允许评分范围时，进行相应修正：例如，对于EachMovie的0到1评分范围，预测值1.2被解释为1。由于MovieLens的评分范围是EachMovie的4倍，MovieLens上的MAE除以4以使结果直接可比。
