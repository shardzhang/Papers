...
点击率（CTR）预估是推荐系统中的核心任务，其目标是估计用户点击推荐 item 的概率。在许多推荐系统中，目标是最大化点击量，因此返回给用户的 item 可以按预估 CTR 排序；而在在线广告等其他应用场景中，提升收益同样重要，因此排序策略可以调整为所有候选 item 的 CTR $\times$ 出价，其中"出价"是用户点击 item 后系统获得的收益。无论哪种情况，关键都在于准确预估 CTR。
...
$$
 \hat{y} = \text{sigmoid}(y_{FM} + y_{DNN}) \qquad (1)
$$
...
$$
 y_{FM} = \langle \mathbf{w}, \mathbf{x} \rangle + \sum_{j_1=1}^{d} \sum_{j_2=j_1+1}^{d} \langle \mathbf{V}_{j_1}, \mathbf{V}_{j_2} \rangle x_{j_1} \cdot x_{j_2} \qquad (2)
$$
...
$$
 \mathbf{a}^{(0)} = [\mathbf{e}_1, \mathbf{e}_2, \dots, \mathbf{e}_m] \qquad (3)
$$

其中 $\mathbf{e}_i$ 是第 $i$ 个字段的嵌入， $m$ 是字段数量。然后 $\mathbf{a}^{(0)}$ 被送入深度神经网络，前向过程为：

$$
 \mathbf{a}^{(l+1)} = \sigma(\mathbf{W}^{(l)} \mathbf{a}^{(l)} + \mathbf{b}^{(l)}) \qquad (4)
$$
...
|------|---------|---------|-----------|------------|
| FNN | $\times$ | $\checkmark$ | $\times$ | $\checkmark$ |
| PNN | $\times$ | $\checkmark$ | $\checkmark$ | $\checkmark$ |
| Wide & Deep | $\checkmark$ | $\checkmark$ | $\checkmark$ | $\times$ |
| DeepFM | $\checkmark$ | $\checkmark$ | $\checkmark$ | $\checkmark$ |
...
2) **公司数据集**：为验证 DeepFM 在实际工业 CTR 预测中的性能，我们在公司*数据集上进行了实验。我们从公司*应用商店的游戏中心收集了连续 7 天的用户点击记录用于训练，接下 1 天的数据用于测试。整个收集的数据集中约有 10 亿条记录。该数据集中包含应用特征（如标识、类别等）、用户特征（如用户下载的应用等）和上下文特征（如操作时间等）。
