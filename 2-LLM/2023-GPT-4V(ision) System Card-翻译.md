...
![图 1：文本截图越狱提示示例。GPT-4V-Early 展示了模型对此类提示的早期性能，GPT-4V Launch 展示了我们正在发布的模型的性能。]()
...
![图 2：持续的安全进展、以额外安全训练数据形式的模型级缓解措施以及系统级缓解措施的结合，在拒绝不允许的提示方面取得了显著进展。]()
...
![图 4：GPT-4V 在尝试处理复杂图像时犯的错误，例如合并术语和遗漏符号。参见附录 A.2 中提供给模型的清晰图像。]()

![图 5：GPT-4V 提供错误指令以合成危险化合物的示例。]()

![图 6：GPT-4V 在正确识别化学结构或有毒食物方面表现不可靠的示例。]()
...
![图 8：早期版本 GPT-4V 表现出的无根据推断和刻板印象示例，以及发布模型所表现的行为对比。³]()
...
![图 10a：GPT-4V 回应了图像的历史含义，但不知道该图像已被仇恨团体挪用。]()

![图 10b：如果被提示，GPT-4V 可以生成内容赞美某些不太知名的仇恨团体，以回应其符号。]()
...
![图 11：GPT-4V 表现出的视觉漏洞示例。此示例表明模型生成对图像提供给模型的顺序敏感。]()
...
![图 12：提供给 GPT-4 的提示示例，用于查找可以用图像替换的短语，以将纯文本提示转换为多模态提示。]()
...
![图 13：模型正确识别个体种族、性别和年龄的能力在不同特征间相似。误差条表示任何种族、性别或年龄的最小和最大性能。]()

![图 14：模型从图像中正确区分个体身份的能力如上所示。我们在两种设置中分析这一点：给定参考图像，个体能否在一张或多张照片中被识别；以及模型能否从单张图像中无条件识别著名的名人和政治家。]()

### A.2

![图 15：图 4 中提供给模型的清晰图像。]()
